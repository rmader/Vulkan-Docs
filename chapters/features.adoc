...
If the application wishes to use a slink:VkDevice with any features
described by sname:{refpage}, it must: add an instance of the structure,
with the desired feature members set to ename:VK_TRUE, to the pname:pNext
chain of slink:VkDeviceCreateInfo when creating the slink:VkDevice.
// end::features[]
endif::hidden[]
...
_Features_ advertise additional functionality which can: be enabled in the
API.
If a feature is _supported_ by a physical device, it must: be _enabled_ when
creating a slink:VkDevice in order to use its associated functionality with
that slink:VkDevice.
If a feature is _unsupported_, that functionality cannot: be used with that
slink:VkDevice.
...
[[features-extension-aliases-table]]
...
| `apiext:VK_KHR_push_descriptor`       | <<features-pushDescriptor, pname:pushDescriptor>>
...
If a member of pname:pEnabledFeatures is ename:VK_TRUE or ename:VK_FALSE,
...
Each structure in pname:pFeatures and its pname:pNext chain contains
basetype:VkBool32 members corresponding to fine-grained features.
Each such member is returned with a ename:VK_TRUE value indicating that
feature is supported on this physical device, or a ename:VK_FALSE value
...
This structure describes the following features:
  ...
  * [[features-robustBufferAccess]] pname:robustBufferAccess enables
    <<shaders-robust-buffer-access>> guarantees for shader buffer accesses.
  ...
  * [[features-sampleRateShading]] pname:sampleRateShading specifies whether
    ...
    the slink:VkPipelineMultisampleStateCreateInfo structure must: be
  ...
  * [[features-logicOp]] pname:logicOp specifies whether logic operations
    ...
    slink:VkPipelineColorBlendStateCreateInfo structure must: be
  ...
  * [[features-depthBiasClamp]] pname:depthBiasClamp specifies whether depth
    ...
    slink:VkPipelineRasterizationStateCreateInfo structure must: be 0.0
    unless the ename:VK_DYNAMIC_STATE_DEPTH_BIAS dynamic state is enabled,
    in which case the pname:depthBiasClamp parameter to
    flink:vkCmdSetDepthBias must: be 0.0.
  ...
  * [[features-wideLines]] pname:wideLines specifies whether lines with
    ...
    slink:VkPipelineRasterizationStateCreateInfo structure must: be 1.0
    unless the ename:VK_DYNAMIC_STATE_LINE_WIDTH dynamic state is enabled,
    in which case the pname:lineWidth parameter to flink:vkCmdSetLineWidth
    must: be 1.0.
  ...
  ** The pname:viewportCount and pname:scissorCount members of the
     slink:VkPipelineViewportStateCreateInfo structure must: be 1.
  ** The pname:firstViewport and pname:viewportCount parameters to the
     flink:vkCmdSetViewport command must: be 0 and 1, respectively.
  ** The pname:firstScissor and pname:scissorCount parameters to the
     flink:vkCmdSetScissor command must: be 0 and 1, respectively.
ifdef::VK_NV_scissor_exclusive[]
  ...
  ** The pname:firstExclusiveScissor and pname:exclusiveScissorCount
     parameters to the flink:vkCmdSetExclusiveScissorNV command must: be 0
     and 1, respectively.
...
supported properties of individual formats as normal.
  ...
  * [[features-shaderImageGatherExtended]] pname:shaderImageGatherExtended
    ...
    If this feature is not enabled, the code:OpImage*Gather
ifdef::VK_QCOM_image_processing3[and code:OpImageGatherQCOM]
    instructions do not support the code:Offset and code:ConstOffsets
    operands.
ifdef::VK_QCOM_image_processing3[]
    The code:OpImageGatherQCOM instruction does not support the
    code:ConstOffsets operand regardless of the value of this feature.
endif::VK_QCOM_image_processing3[]
    This also specifies whether shader modules can: declare the
...
====
  ...
  * [[features-shaderStorageImageMultisample]]
    ...
    ename:VK_IMAGE_USAGE_STORAGE_BIT usage flag set must: be created with
    pname:samples equal to ename:VK_SAMPLE_COUNT_1_BIT.
    This also specifies whether shader modules can: declare the
...
include::{chapters}/features.adoc[tag=VK_KHR_shader_float16_int8-features]
  * [[features-descriptorIndexing]] pname:descriptorIndexing indicates
    ...
    Enabling this feature when flink:vkCreateDevice is called does not imply
...
include::{chapters}/features.adoc[tag=VK_KHR_buffer_device_address-features]
...
[open,refpage='VkPhysicalDeviceVulkan14Features',desc='Structure describing the Vulkan 1.4 features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVulkan14Features structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceVulkan14Features.adoc[]
...
:anchor-prefix:
include::{chapters}/features.adoc[tag=VK_KHR_global_priority-features]
include::{chapters}/features.adoc[tag=VK_KHR_shader_subgroup_rotate-features]
include::{chapters}/features.adoc[tag=VK_KHR_shader_float_controls2-features]
include::{chapters}/features.adoc[tag=VK_KHR_shader_expect_assume-features]
include::{chapters}/features.adoc[tag=VK_KHR_line_rasterization-features]
include::{chapters}/features.adoc[tag=VK_KHR_vertex_attribute_divisor-features]
include::{chapters}/features.adoc[tag=VK_KHR_index_type_uint8-features]
include::{chapters}/features.adoc[tag=VK_KHR_dynamic_rendering_local_read-features]
include::{chapters}/features.adoc[tag=VK_KHR_maintenance5-features]
include::{chapters}/features.adoc[tag=VK_KHR_maintenance6-features]
include::{chapters}/features.adoc[tag=VK_EXT_pipeline_protected_access-features]
include::{chapters}/features.adoc[tag=VK_EXT_pipeline_robustness-features]
include::{chapters}/features.adoc[tag=VK_EXT_host_image_copy-features]
  * [[features-pushDescriptor]] pname:pushDescriptor indicates that the
    implementation supports push descriptors.

:refpage: VkPhysicalDeviceVulkan14Features
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVulkan14Features.adoc[]
--
endif::VK_BASE_VERSION_1_4[]

ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_variable_pointers[]
...
ifdef::VK_COMPUTE_VERSION_1_2[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_2[:anchor-prefix:]
...
--
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_variable_pointers[]
...
// Must have preceding whitespace
ifdef::VK_GRAPHICS_VERSION_1_2[:anchor-prefix: extension-]
ifndef::VK_GRAPHICS_VERSION_1_2[:anchor-prefix:]
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_atomic_int64[]
[open,refpage='VkPhysicalDeviceShaderAtomicInt64Features',desc='Structure describing features supported by VK_KHR_shader_atomic_int64',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_atomic_int64[]
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_8bit_storage[]
[open,refpage='VkPhysicalDevice8BitStorageFeatures',desc='Structure describing features supported by VK_KHR_8bit_storage',type='structs']
--
...
ifdef::VK_KHR_shader_untyped_pointers[]
    objects unless the <<features-shaderUntypedPointers,
    pname:shaderUntypedPointer>> feature is enabled and they are accessed in
    32-bit multiples or 16-bit multiples if the
    <<{anchor-prefix}features-storageBuffer16BitAccess,
    pname:storageBuffer16BitAccess>> feature is enabled.
endif::VK_KHR_shader_untyped_pointers[]
...
ifdef::VK_KHR_shader_untyped_pointers[]
    objects unless the <<features-shaderUntypedPointers,
    pname:shaderUntypedPointers>> feature is enabled and they are accessed
    in 32-bit multiples or 16-bit multiples if the
    <<{anchor-prefix}features-uniformAndStorageBuffer16BitAccess,
    pname:uniformAndStorageBuffer16BitAccess>> feature is enabled.
endif::VK_KHR_shader_untyped_pointers[]
    This also indicates whether shader modules can: declare the
...
ifdef::VK_KHR_shader_untyped_pointers[]
    ...
    pname:storagePushConstant16>> feature is enabled.
endif::VK_KHR_shader_untyped_pointers[]
    This also indicates whether shader modules can: declare the
...
--
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_8bit_storage[]
...
ifdef::VK_KHR_shader_untyped_pointers[]
    objects unless the <<{anchor-prefix}features-storageBuffer8BitAccess,
    pname:storageBuffer8BitAccess>> or
    <<{anchor-prefix}features-uniformAndStorageBuffer8BitAccess,
    pname:uniformAndStorageBuffer8BitAccess>> features are enabled or they
    are accessed in 32-bit multiples if the
    <<features-shaderUntypedPointers, pname:shaderUntypedPointers>> feature
    is enabled.
endif::VK_KHR_shader_untyped_pointers[]
    This also specifies whether shader modules can: declare the
...
ifdef::VK_KHR_shader_untyped_pointers[]
    objects unless the
    <<{anchor-prefix}features-uniformAndStorageBuffer8BitAccess,
    pname:uniformAndStorageBuffer8BitAccess>> are enabled or they are
    accessed in 32-bit multiples if the <<features-shaderUntypedPointers,
    pname:shaderUntypedPointers>> feature is enabled.
endif::VK_KHR_shader_untyped_pointers[]
    This also specifies whether shader modules can: declare the
...
ifdef::VK_KHR_shader_untyped_pointers[]
    objects unless the <<{anchor-prefix}features-storagePushConstant8,
    pname:storagePushConstant8>> feature is enabled or they are accessed in
    32-bit multiples if the <<features-shaderUntypedPointers,
    pname:shaderUntypedPointers>> feature is enabled.
endif::VK_KHR_shader_untyped_pointers[]
    This also specifies whether shader modules can: declare the
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_float16_int8[]
[open,refpage='VkPhysicalDeviceShaderFloat16Int8Features',desc='Structure describing features supported by VK_KHR_shader_float16_int8',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_float16_int8[]
...
[open,refpage='VkPhysicalDeviceSamplerYcbcrConversionFeatures',desc='Structure describing {YCbCr} conversion features that can be supported by an implementation',type='structs']
--
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-taskShaderNV]] pname:taskShader specifies whether task
    ...
    used.
  * [[features-meshShaderNV]] pname:meshShader specifies whether mesh
    shaders are supported.
...
include::{generated}/api/structs/VkPhysicalDeviceMeshShaderFeaturesEXT.adoc[]
  ...
  * [[features-meshShader]] pname:meshShader specifies whether mesh shaders
    ...
    used.
ifdef::VK_GRAPHICS_VERSION_1_1,VK_KHR_multiview[]
  * [[features-multiviewMeshShader]] pname:multiviewMeshShader specifies
    whether the implementation supports <<features-multiview,
    pname:multiview>> rendering within a render pass, with mesh shaders.
    If this feature is not enabled, then a pipeline compiled against a
...
ifdef::VK_NV_memory_decompression,VK_EXT_memory_decompression[]
[open,refpage='VkPhysicalDeviceMemoryDecompressionFeaturesEXT',desc='Structure describing if memory decompression is supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceMemoryDecompressionFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceMemoryDecompressionFeaturesEXT.adoc[]
...
:refpage: VkPhysicalDeviceMemoryDecompressionFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMemoryDecompressionFeaturesEXT.adoc[]
--
endif::VK_NV_memory_decompression,VK_EXT_memory_decompression[]

ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_descriptor_indexing[]
[open,refpage='VkPhysicalDeviceDescriptorIndexingFeatures',desc='Structure describing descriptor indexing features that can be supported by an implementation',type='structs']
--
...
This structure describes the following features:
  ...
  * [[features-indirectCopy]] pname:indirectCopy indicates whether
    <<indirect-copies, indirect memory to memory or, memory to image
...
[open,refpage='VkPhysicalDeviceCopyMemoryIndirectFeaturesKHR',desc='Structure describing indirect copy features supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCopyMemoryIndirectFeaturesKHR structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceCopyMemoryIndirectFeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-indirectMemoryCopy]] pname:indirectMemoryCopy indicates
    whether <<indirect-copies, indirect memory to memory copies>> are
    supported.
  * [[features-indirectMemoryToImageCopy]] pname:indirectMemoryToImageCopy
    indicates whether <<indirect-copies, indirect memory to image copies>>
    are supported.

:refpage: VkPhysicalDeviceCopyMemoryIndirectFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCopyMemoryIndirectFeaturesKHR.adoc[]
--
endif::VK_KHR_copy_memory_indirect[]

ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_vertex_attribute_divisor,VK_EXT_vertex_attribute_divisor[]
[open,refpage='VkPhysicalDeviceVertexAttributeDivisorFeatures',desc='Structure describing if fetching of vertex attribute may be repeated for instanced rendering',type='structs']
--
The sname:VkPhysicalDeviceVertexAttributeDivisorFeatures structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVertexAttributeDivisorFeatures.adoc[]
...
// Must have preceding whitespace
ifdef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_vertex_attribute_divisor-features[]
  * [[{anchor-prefix}features-vertexAttributeInstanceRateDivisor]]
  ...
  * [[{anchor-prefix}features-vertexAttributeInstanceRateZeroDivisor]]
    pname:vertexAttributeInstanceRateZeroDivisor specifies whether a zero
    value for slink:VkVertexInputBindingDivisorDescription::pname:divisor is
    supported.
// end::VK_KHR_vertex_attribute_divisor-features[]

:refpage: VkPhysicalDeviceVertexAttributeDivisorFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVertexAttributeDivisorFeatures.adoc[]
--
endif::VK_GRAPHICS_VERSION_1_4,VK_KHR_vertex_attribute_divisor,VK_EXT_vertex_attribute_divisor[]
...
// Must have preceding whitespace
ifdef::VK_BASE_VERSION_1_2[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_2[:anchor-prefix:]
...
[open,refpage='VkPhysicalDeviceInlineUniformBlockFeatures',desc='Structure describing inline uniform block features that can be supported by an implementation',type='structs']
--
...
// Must have preceding whitespace
ifdef::VK_COMPUTE_VERSION_1_3[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_3[:anchor-prefix:]
...
[open,refpage='VkPhysicalDeviceComputeShaderDerivativesFeaturesKHR',desc='Structure describing compute shader derivative features that can be supported by an implementation',type='structs']
--
...
[open,refpage='VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT',desc='Structure describing fragment density map offset features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT.adoc[]
...
:refpage: VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceFragmentDensityMapOffsetFeaturesEXT.adoc[]
--
endif::VK_EXT_fragment_density_map_offset,VK_QCOM_fragment_density_map_offset[]

ifdef::VK_VALVE_fragment_density_map_layered[]
[open,refpage='VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE',desc='Structure describing additional layered fragment density map features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-fragmentDensityMapLayered]] pname:fragmentDensityMapLayered
    specifies whether the implementation supports layered fragment density
    maps.

:refpage: VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE.adoc[]
--
...
[open,refpage='VkPhysicalDeviceScalarBlockLayoutFeatures',desc='Structure indicating support for scalar block layouts',type='structs']
--
...
[open,refpage='VkPhysicalDeviceUniformBufferStandardLayoutFeatures',desc='Structure indicating support for std430-like packing in uniform buffers',type='structs']
--
...
[open,refpage='VkPhysicalDeviceBufferDeviceAddressFeatures',desc='Structure describing buffer address features that can be supported by an implementation',type='structs']
--
...
ifdef::VK_BASE_VERSION_1_2[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_2[:anchor-prefix:]
// tag::VK_KHR_buffer_device_address-features[]
...
// end::VK_KHR_buffer_device_address-features[]
...
This structure describes the following features:
  ...
  * [[features-bufferDeviceAddressMultiDeviceEXT]]
    pname:bufferDeviceAddressMultiDevice indicates that the implementation
    supports the <<features-bufferDeviceAddressEXT,
    pname:bufferDeviceAddress>> feature for logical devices created with
...
ifdef::VK_QCOM_elapsed_timer_query[]
[open,refpage='VkPhysicalDeviceElapsedTimerQueryFeaturesQCOM',desc='Structure describing elapsed timer query features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceElapsedTimerQueryFeaturesQCOM structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceElapsedTimerQueryFeaturesQCOM.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-elapsedTimerQuery]] pname:elapsedTimerQuery indicates that
    the implementation supports ename:VK_QUERY_TYPE_TIME_ELAPSED_QCOM
    queries.

:refpage: VkPhysicalDeviceElapsedTimerQueryFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceElapsedTimerQueryFeaturesQCOM.adoc[]
--
endif::VK_QCOM_elapsed_timer_query[]

ifdef::VK_GRAPHICS_VERSION_1_2,VK_KHR_imageless_framebuffer[]
[open,refpage='VkPhysicalDeviceImagelessFramebufferFeatures',desc='Structure indicating support for imageless framebuffers',type='structs']
--
...
--
endif::VK_GRAPHICS_VERSION_1_2,VK_KHR_imageless_framebuffer[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-cooperativeMatrixNV]] pname:cooperativeMatrix indicates that
  ...
  * [[features-cooperativeMatrixRobustBufferAccessNV]]
    ...
    <<shaders-robust-buffer-access, robust buffer access>> for SPIR-V
...
This structure describes the following features:
  ...
  * [[features-cooperativeMatrixRobustBufferAccess]]
    pname:cooperativeMatrixRobustBufferAccess indicates that the
    implementation supports <<shaders-robust-buffer-access, robust buffer
    access>> for SPIR-V code:OpCooperativeMatrixLoadKHR and
    code:OpCooperativeMatrixStoreKHR instructions.
...
[open,refpage='VkPhysicalDeviceCooperativeMatrixConversionFeaturesQCOM',desc='Structure describing cooperative matrix conversion features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCooperativeMatrixConversionFeaturesQCOM structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceCooperativeMatrixConversionFeaturesQCOM.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-cooperativeMatrixConversion]]
    pname:cooperativeMatrixConversion indicates that the implementation
    supports the code:CooperativeMatrixConversionQCOM SPIR-V capability.

:refpage: VkPhysicalDeviceCooperativeMatrixConversionFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCooperativeMatrixConversionFeaturesQCOM.adoc[]
--
endif::VK_QCOM_cooperative_matrix_conversion[]

ifdef::VK_NV_cooperative_matrix2[]
[open,refpage='VkPhysicalDeviceCooperativeMatrix2FeaturesNV',desc='Structure describing cooperative matrix features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCooperativeMatrix2FeaturesNV structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceCooperativeMatrix2FeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * [[features-cooperativeMatrixWorkgroupScope]]
    pname:cooperativeMatrixWorkgroupScope indicates that the implementation
    supports workgroup scope cooperative matrices.
  * [[features-cooperativeMatrixFlexibleDimensions]]
    pname:cooperativeMatrixFlexibleDimensions indicates that the
    implementation supports cooperative matrix sizes that are a multiple of
    the granularity advertised in
    slink:VkCooperativeMatrixFlexibleDimensionsPropertiesNV.
  * [[features-cooperativeMatrixReductions]]
    pname:cooperativeMatrixReductions indicates that the implementation
    supports the code:CooperativeMatrixReductionsNV SPIR-V capability.
    This allows performing (row, column, 2x2, or all element) reductions on
    matrices.
  * [[features-cooperativeMatrixConversions]]
    pname:cooperativeMatrixConversions indicates that the implementation
    supports the code:CooperativeMatrixConversionsNV SPIR-V capability.
    This allows converting accumulator matrices to A or B matrices.
  * [[features-cooperativeMatrixPerElementOperations]]
    pname:cooperativeMatrixPerElementOperations indicates that the
    implementation supports the code:CooperativeMatrixPerElementOperationsNV
    SPIR-V capability.
    This allows performing element-wise operations on matrix elements using
    a callback function.
  * [[features-cooperativeMatrixTensorAddressing]]
    pname:cooperativeMatrixTensorAddressing indicates that the
    implementation supports the code:TensorAddressingNV and
    code:CooperativeMatrixTensorAddressingNV SPIR-V capabilities.
    This allows using tensor layout and tensor view types for matrix loads
    and stores.
  * [[features-cooperativeMatrixBlockLoads]]
    pname:cooperativeMatrixBlockLoads indicates that the implementation
    supports the code:CooperativeMatrixBlockLoadsNV SPIR-V capability.
    This allows setting block size for loads and using a callback function
    to decode block elements.

:refpage: VkPhysicalDeviceCooperativeMatrix2FeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCooperativeMatrix2FeaturesNV.adoc[]
--
endif::VK_NV_cooperative_matrix2[]

ifdef::VK_NV_cooperative_matrix_decode_vector[]
[open,refpage='VkPhysicalDeviceCooperativeMatrixDecodeVectorFeaturesNV',desc='Structure describing cooperative matrix decode vector features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCooperativeMatrixDecodeVectorFeaturesNV structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceCooperativeMatrixDecodeVectorFeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-cooperativeMatrixDecodeVector]]
    pname:cooperativeMatrixDecodeVector indicates that the implementation
    supports the code:CooperativeMatrixDecodeVectorNV SPIR-V capability.
    This allows code:OpCooperativeMatrixLoadTensorNV to use an optional
    *DecodeVectorFunc* operand to decode multiple block-adjacent elements
    per callback, as defined in
    code:SPV_NV_cooperative_matrix_decode_vector.

:refpage: VkPhysicalDeviceCooperativeMatrixDecodeVectorFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCooperativeMatrixDecodeVectorFeaturesNV.adoc[]
--
endif::VK_NV_cooperative_matrix_decode_vector[]

ifdef::VK_EXT_cooperative_matrix_maintenance1[]
[open,refpage='VkPhysicalDeviceCooperativeMatrixMaintenance1FeaturesEXT',desc='Structure describing cooperative matrix features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCooperativeMatrixMaintenance1FeaturesEXT structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceCooperativeMatrixMaintenance1FeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-cooperativeMatrixProperties2EXT]]
    pname:cooperativeMatrixProperties2 indicates that the implementation
    supports the flink:vkGetPhysicalDeviceCooperativeMatrixProperties2EXT
    command.
  * [[features-cooperativeMatrixReductionsEXT]]
    pname:cooperativeMatrixReductions indicates that the implementation
    supports the code:CooperativeMatrixReductionsEXT SPIR-V capability.
    This allows performing (row, column, 2x2, or all element) reductions on
    matrices.
  * [[features-cooperativeMatrixConversionsEXT]]
    pname:cooperativeMatrixConversions indicates that the implementation
    supports the code:CooperativeMatrixConversionsEXT SPIR-V capability.
    This allows converting accumulator matrices to A or B matrices.
  * [[features-cooperativeMatrixPerElementOperationsEXT]]
    pname:cooperativeMatrixPerElementOperations indicates that the
    implementation supports the
    code:CooperativeMatrixPerElementOperationsEXT SPIR-V capability.
    This allows performing element-wise operations on matrix elements using
    a callback function.
  * [[features-cooperativeMatrixGetCoordinateEXT]]
    pname:cooperativeMatrixGetCoordinate indicates that the implementation
    supports the code:CooperativeMatrixGetCoordinateEXT SPIR-V capability.
    This allows querying locations for a matrix element.

:refpage: VkPhysicalDeviceCooperativeMatrixMaintenance1FeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCooperativeMatrixMaintenance1FeaturesEXT.adoc[]
--
endif::VK_EXT_cooperative_matrix_maintenance1[]

ifdef::VK_NV_cooperative_vector[]
[open,refpage='VkPhysicalDeviceCooperativeVectorFeaturesNV',desc='Structure describing cooperative vector features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceCooperativeVectorFeaturesNV structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceCooperativeVectorFeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-cooperativeVector]] pname:cooperativeVector indicates that
    the implementation supports the code:CooperativeVectorNV SPIR-V
    capability.
  * [[features-cooperativeVectorTraining]] pname:cooperativeVectorTraining
    indicates that the implementation supports the
    code:CooperativeVectorTrainingNV SPIR-V capability.

:refpage: VkPhysicalDeviceCooperativeVectorFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCooperativeVectorFeaturesNV.adoc[]
--
endif::VK_NV_cooperative_vector[]

ifdef::VK_EXT_shader_long_vector[]
[open,refpage='VkPhysicalDeviceShaderLongVectorFeaturesEXT',desc='Structure describing long vector features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderLongVectorFeaturesEXT structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderLongVectorFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-longVector]] pname:longVector indicates that the
    implementation supports the code:LongVectorEXT SPIR-V capability.

:refpage: VkPhysicalDeviceShaderLongVectorFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderLongVectorFeaturesEXT.adoc[]
--
...
ifdef::VK_COMPUTE_VERSION_1_1[]
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_subgroup_extended_types[]
[open,refpage='VkPhysicalDeviceShaderSubgroupExtendedTypesFeatures',desc='Structure describing the extended types subgroups support feature for an implementation',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_shader_subgroup_extended_types[]
endif::VK_COMPUTE_VERSION_1_1[]

ifdef::VK_BASE_VERSION_1_2,VK_EXT_host_query_reset[]
[open,refpage='VkPhysicalDeviceHostQueryResetFeatures',desc='Structure describing whether queries can be reset from the host',type='structs']
--
...
--
endif::VK_BASE_VERSION_1_2,VK_EXT_host_query_reset[]
...
[open,refpage='VkPhysicalDeviceTimelineSemaphoreFeatures',desc='Structure describing timeline semaphore features that can be supported by an implementation',type='structs']
--
...
[open,refpage='VkPhysicalDeviceInternallySynchronizedQueuesFeaturesKHR',desc='Structure indicating support for queues that are internally synchronized',type='structs']
--
The sname:VkPhysicalDeviceInternallySynchronizedQueuesFeaturesKHR structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceInternallySynchronizedQueuesFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * [[features-internallySynchronizedQueues]]
    pname:internallySynchronizedQueues indicates that
    ename:VK_DEVICE_QUEUE_CREATE_INTERNALLY_SYNCHRONIZED_BIT_KHR can be used
    to make queues internally synchronized.

:refpage: VkPhysicalDeviceInternallySynchronizedQueuesFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceInternallySynchronizedQueuesFeaturesKHR.adoc[]
--
...
describe the following features:
  ...
  * [[features-sciSyncImport]] pname:sciSyncImport indicates whether
    ...
    If pname:sciSyncImport is ename:VK_TRUE, slink:VkFence and/or
  ...
  * [[features-sciSyncExport]] pname:sciSyncExport indicates whether
    ...
    If pname:sciSyncExport is ename:VK_TRUE, slink:VkFence and/or
...
structure describe the following features:
  ...
  * [[features-sciSyncExport2]] pname:sciSyncExport indicates whether
    stext:NvSciSyncObj export functionality is supported.
    If pname:sciSyncExport is ename:VK_TRUE, slink:VkFence supports
...
structure describe the following features:
  ...
  * [[features-sciBufImport]] pname:sciBufImport indicates whether
    ...
    If pname:sciBufImport is ename:VK_TRUE, slink:VkDeviceMemory supports
    importing stext:NvSciBufObj from applications.
    In this case, the application is responsible for the resource management
  ...
  * [[features-sciBufExport]] pname:sciBufExport indicates whether
    ...
    If pname:sciBufExport is ename:VK_TRUE, slink:VkDeviceMemory supports
    exporting stext:NvSciBufObj created by the driver to applications.
    In this case, the driver is responsible for the resource management of
...
describe the following features:
  ...
  * [[features-screenBufferImport]] pname:screenBufferImport indicates
    ...
    If pname:screenBufferImport is ename:VK_TRUE, slink:VkDeviceMemory
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_index_type_uint8,VK_EXT_index_type_uint8[]
[open,refpage='VkPhysicalDeviceIndexTypeUint8Features',desc='Structure describing whether uint8 index type can be used',type='structs']
--
The sname:VkPhysicalDeviceIndexTypeUint8Features structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceIndexTypeUint8Features.adoc[]
...
// Must have preceding whitespace
ifdef::VK_BASE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_index_type_uint8-features[]
  * [[{anchor-prefix}features-indexTypeUint8]] pname:indexTypeUint8
    indicates that ename:VK_INDEX_TYPE_UINT8 can be used with
ifdef::VK_BASE_VERSION_1_4,VK_KHR_maintenance5[flink:vkCmdBindIndexBuffer2 and]
    flink:vkCmdBindIndexBuffer.
// end::VK_KHR_index_type_uint8-features[]

:refpage: VkPhysicalDeviceIndexTypeUint8Features
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceIndexTypeUint8Features.adoc[]
--
endif::VK_BASE_VERSION_1_4,VK_KHR_index_type_uint8,VK_EXT_index_type_uint8[]
...
[open,refpage='VkPhysicalDeviceSeparateDepthStencilLayoutsFeatures',desc='Structure describing whether the implementation can do depth and stencil image barriers separately',type='structs']
--
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_shader_demote_to_helper_invocation[]
[open,refpage='VkPhysicalDeviceShaderDemoteToHelperInvocationFeatures',desc='Structure describing the shader demote to helper invocations features that can be supported by an implementation',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_3,VK_EXT_shader_demote_to_helper_invocation[]
...
[open,refpage='VkPhysicalDeviceTextureCompressionASTCHDRFeatures',desc='Structure describing ASTC HDR features that can be supported by an implementation',type='structs']
--
...
// Must have preceding whitespace
ifdef::VK_BASE_VERSION_1_3[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_3[:anchor-prefix:]
...
[open,refpage='VkPhysicalDeviceTextureCompressionASTC3DFeaturesEXT',desc='Structure describing ASTC 3D features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceTextureCompressionASTC3DFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceTextureCompressionASTC3DFeaturesEXT.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-textureCompressionASTC_3D]] pname:textureCompressionASTC_3D
    ...
    be supported in pname:optimalTilingFeatures for the following formats:
+
  ** ename:VK_FORMAT_ASTC_3x3x3_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_3x3x3_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_3x3x3_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x3x3_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x3x3_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x3x3_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x3_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x3_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x3_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x4_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x4_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_4x4x4_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x4x4_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x4x4_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x4x4_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x4_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x4_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x4_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x5_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x5_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_5x5x5_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x5x5_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x5x5_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x5x5_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x5_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x5_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x5_SFLOAT_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x6_UNORM_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x6_SRGB_BLOCK_EXT
  ** ename:VK_FORMAT_ASTC_6x6x6_SFLOAT_BLOCK_EXT

:refpage: VkPhysicalDeviceTextureCompressionASTC3DFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceTextureCompressionASTC3DFeaturesEXT.adoc[]
--
endif::VK_EXT_texture_compression_astc_3d[]

ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_line_rasterization,VK_EXT_line_rasterization[]
[open,refpage='VkPhysicalDeviceLineRasterizationFeatures',desc='Structure describing the line rasterization features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceLineRasterizationFeatures structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceLineRasterizationFeatures.adoc[]
...
ifdef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_line_rasterization-features[]
  * [[{anchor-prefix}features-rectangularLines]] pname:rectangularLines
    indicates whether the implementation supports
    <<primsrast-lines,rectangular line rasterization>>.
  * [[{anchor-prefix}features-bresenhamLines]] pname:bresenhamLines
    indicates whether the implementation supports
    <<primsrast-lines-bresenham,Bresenham-style line rasterization>>.
  * [[{anchor-prefix}features-smoothLines]] pname:smoothLines indicates
    whether the implementation supports <<primsrast-lines-smooth,smooth line
    rasterization>>.
  * [[{anchor-prefix}features-stippledRectangularLines]]
    ...
    ename:VK_LINE_RASTERIZATION_MODE_RECTANGULAR lines.
  * [[{anchor-prefix}features-stippledBresenhamLines]]
    pname:stippledBresenhamLines indicates whether the implementation
    supports <<primsrast-lines-stipple,stippled line rasterization>> with
    ename:VK_LINE_RASTERIZATION_MODE_BRESENHAM lines.
  * [[{anchor-prefix}features-stippledSmoothLines]]
    pname:stippledSmoothLines indicates whether the implementation supports
    <<primsrast-lines-stipple,stippled line rasterization>> with
    ename:VK_LINE_RASTERIZATION_MODE_RECTANGULAR_SMOOTH lines.
// end::VK_KHR_line_rasterization-features[]

:refpage: VkPhysicalDeviceLineRasterizationFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceLineRasterizationFeatures.adoc[]
--
...
[open,refpage='VkPhysicalDeviceSubgroupSizeControlFeatures',desc='Structure describing the subgroup size control features that can be supported by an implementation',type='structs']
--
...
ifdef::VK_KHR_video_encode_feedback2[]
[open,refpage='VkPhysicalDeviceVideoEncodeFeedback2FeaturesKHR',desc='Structure describing additional video encode feedback features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoEncodeFeedback2FeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoEncodeFeedback2FeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoEncodeFeedback2]] pname:videoEncodeFeedback2 specifies
    that the implementation supports
    <<queries-video-encode-per-partition-feedback,per picture partition
    feedback>> and the following additional
    <<queries-video-encode-feedback,video encode feedback flags>>, as
    reported in slink:VkVideoEncodeFeedback2CapabilitiesKHR and/or
    slink:VkVideoEncodeCapabilitiesKHR::pname:supportedEncodeFeedbackFlags,
    respectively:
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_AVERAGE_QUANTIZATION_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_MIN_QUANTIZATION_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_MAX_QUANTIZATION_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR.
  ** ename:VK_VIDEO_ENCODE_FEEDBACK_PICTURE_PARTITION_COUNT_BIT_KHR.
+
[NOTE]
====
Support for pname:videoEncodeFeedback2 does not indicate that all video
...
:refpage: VkPhysicalDeviceVideoEncodeFeedback2FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoEncodeFeedback2FeaturesKHR.adoc[]
--
endif::VK_KHR_video_encode_feedback2[]

ifdef::VK_KHR_video_encode_quantization_map[]
[open,refpage='VkPhysicalDeviceVideoEncodeQuantizationMapFeaturesKHR',desc='Structure describing the video encode quantization map features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoEncodeQuantizationMapFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoEncodeQuantizationMapFeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoEncodeQuantizationMap]] pname:videoEncodeQuantizationMap
    indicates that the implementation supports
    <<encode-quantization-map,video encode quantization maps>>.
+
[NOTE]
====
Support for pname:videoEncodeQuantizationMap does not indicate that all
...
:refpage: VkPhysicalDeviceVideoEncodeQuantizationMapFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoEncodeQuantizationMapFeaturesKHR.adoc[]
--
endif::VK_KHR_video_encode_quantization_map[]

ifdef::VK_KHR_video_encode_intra_refresh[]
[open,refpage='VkPhysicalDeviceVideoEncodeIntraRefreshFeaturesKHR',desc='Structure describing the video encode intra refresh features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoEncodeIntraRefreshFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoEncodeIntraRefreshFeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoEncodeIntraRefresh]] pname:videoEncodeIntraRefresh
    specifies that the implementation supports <<encode-intra-refresh,video
    encode intra refresh>>.
+
[NOTE]
====
Support for pname:videoEncodeIntraRefresh does not indicate that all video
...
:refpage: VkPhysicalDeviceVideoEncodeIntraRefreshFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoEncodeIntraRefreshFeaturesKHR.adoc[]
--
endif::VK_KHR_video_encode_intra_refresh[]

ifdef::VK_VALVE_video_encode_rgb_conversion[]
[open,refpage='VkPhysicalDeviceVideoEncodeRgbConversionFeaturesVALVE',desc='Structure describing the video encode RGB conversion features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoEncodeRgbConversionFeaturesVALVE structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoEncodeRgbConversionFeaturesVALVE.adoc[]
...
This structure describes the following features:
  ...
  * [[features-videoEncodeRgbConversion]] pname:videoEncodeRgbConversion
    specifies that the implementation supports <<encode-rgb-conversion,video
    encode RGB conversion>>.
+

:refpage: VkPhysicalDeviceVideoEncodeRgbConversionFeaturesVALVE
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoEncodeRgbConversionFeaturesVALVE.adoc[]
--
...
[open,refpage='VkPhysicalDeviceVideoMaintenance2FeaturesKHR',desc='Structure describing the video maintenance features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoMaintenance2FeaturesKHR structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceVideoMaintenance2FeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoMaintenance2]] pname:videoMaintenance2 specifies that
    the implementation supports the following:
ifdef::VK_KHR_video_decode_queue[]
  ** Support for issuing <<video-coding-control,video coding control>>
     commands against video decode sessions without a bound video session
     parameters object.
  ** The new video session creation flag
     ename:VK_VIDEO_SESSION_CREATE_INLINE_SESSION_PARAMETERS_BIT_KHR for
     video decode sessions.
endif::VK_KHR_video_decode_queue[]
ifdef::VK_KHR_video_encode_queue[]
ifdef::VK_KHR_video_encode_h264,VK_KHR_video_encode_h265,VK_KHR_video_encode_av1[]
  ** Required support for the <<encode-rate-control-modes,rate control
     mode>> ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR for the
     following video encode profiles:
ifdef::VK_KHR_video_encode_h264[]
  *** <<encode-h264-profile,H.264 encode profiles>>;
endif::VK_KHR_video_encode_h264[]
ifdef::VK_KHR_video_encode_h265[]
  *** <<encode-h265-profile,H.265 encode profiles>>;
endif::VK_KHR_video_encode_h265[]
ifdef::VK_KHR_video_encode_av1[]
  *** <<encode-av1-profile,AV1 encode profiles>>.
endif::VK_KHR_video_encode_av1[]
endif::VK_KHR_video_encode_h264,VK_KHR_video_encode_h265,VK_KHR_video_encode_av1[]
  ** Additional guarantees on Video Std parameters used with video encode
     ...
     <<encode-overrides,override>> them.
endif::VK_KHR_video_encode_queue[]

:refpage: VkPhysicalDeviceVideoMaintenance2FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoMaintenance2FeaturesKHR.adoc[]
--
endif::VK_KHR_video_maintenance2[]

ifdef::VK_KHR_video_encode_av1[]
[open,refpage='VkPhysicalDeviceVideoEncodeAV1FeaturesKHR',desc='Structure describing the video encode AV1 features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoEncodeAV1FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoEncodeAV1FeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoEncodeAV1]] pname:videoEncodeAV1 indicates that the
    implementation supports <<encode-av1,AV1 encode operations>>.

:refpage: VkPhysicalDeviceVideoEncodeAV1FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoEncodeAV1FeaturesKHR.adoc[]
--
endif::VK_KHR_video_encode_av1[]

ifdef::VK_KHR_video_decode_vp9[]
[open,refpage='VkPhysicalDeviceVideoDecodeVP9FeaturesKHR',desc='Structure describing the video decode VP9 features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVideoDecodeVP9FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceVideoDecodeVP9FeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-videoDecodeVP9]] pname:videoDecodeVP9 specifies that the
    implementation supports <<decode-vp9,VP9 decode operations>>.

:refpage: VkPhysicalDeviceVideoDecodeVP9FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVideoDecodeVP9FeaturesKHR.adoc[]
--
...
[open,refpage='VkPhysicalDeviceExtendedFlagsFeaturesKHR',desc='Structure describing whether the implementation supports extended flags functionality',type='structs']
--
The sname:VkPhysicalDeviceExtendedFlagsFeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceExtendedFlagsFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-extendedFlags]] pname:extendedFlags indicates that the
    implementation supports the following:
  * slink:VkFormatProperties4KHR, extending slink:VkFormatProperties2, and
    corresponding elink:VkFormatFeatureFlagBits4KHR and
    tlink:VkFormatFeatureFlags4KHR types providing up to 64 new format
    feature flags.
  * slink:VkImageCreateFlags2CreateInfoKHR, extending structures passed to
    image create and query functions, and corresponding
    elink:VkImageCreateFlagBits2KHR and tlink:VkImageCreateFlags2KHR types
    providing up to 32 new image create flags.
  * slink:VkImageUsageFlags2CreateInfoKHR, extending structures passed to
    image create and query functions, and corresponding
    elink:VkImageUsageFlagBits2KHR and tlink:VkImageUsageFlags2KHR types
    providing up to 32 new image usage flags.
  * The slink:VkPipelineCreateFlags2CreateInfoKHR,
    tlink:VkPipelineCreateFlags2KHR, elink:VkPipelineCreateFlagBits2KHR,
    slink:VkBufferUsageFlags2CreateInfoKHR, tlink:VkBufferUsageFlags2KHR,
    and elink:VkBufferUsageFlagBits2KHR types originally introduced by
    apiext:VK_KHR_maintenance5.

:refpage: VkPhysicalDeviceExtendedFlagsFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceExtendedFlagsFeaturesKHR.adoc[]
--
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-deviceGeneratedCommandsNV]] pname:deviceGeneratedCommands
...
[open,refpage='VkPhysicalDeviceClusterAccelerationStructureFeaturesNV',desc='Structure describing the ray tracing cluster geometry feature supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceClusterAccelerationStructureFeaturesNV structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceClusterAccelerationStructureFeaturesNV.adoc[]
...
This structure describes the following feature:
  ...
  * [[features-clusterAccelerationStructure]]
    pname:clusterAccelerationStructure indicates whether the implementation
    supports the ability to generate and trace cluster acceleration
    structures.

:refpage: VkPhysicalDeviceClusterAccelerationStructureFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceClusterAccelerationStructureFeaturesNV.adoc[]
--
endif::VK_NV_cluster_acceleration_structure[]

ifdef::VK_NV_partitioned_acceleration_structure[]
[open,refpage='VkPhysicalDevicePartitionedAccelerationStructureFeaturesNV',desc='Structure describing the ray tracing partitioned top level acceleration structure feature supported by an implementation',type='structs']
--
The sname:VkPhysicalDevicePartitionedAccelerationStructureFeaturesNV
structure is defined as:

include::{generated}/api/structs/VkPhysicalDevicePartitionedAccelerationStructureFeaturesNV.adoc[]
...
This structure describes the following feature:
  ...
  * [[features-partitionedAccelerationStructure]]
    pname:partitionedAccelerationStructure indicates whether the
    implementation supports the ability to generate top level partitioned
    acceleration structures.

:refpage: VkPhysicalDevicePartitionedAccelerationStructureFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePartitionedAccelerationStructureFeaturesNV.adoc[]
--
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_global_priority,VK_EXT_global_priority_query[]
[open,refpage='VkPhysicalDeviceGlobalPriorityQueryFeatures',desc='Structure describing whether global priority query can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceGlobalPriorityQueryFeatures structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceGlobalPriorityQueryFeatures.adoc[]
...
ifdef::VK_BASE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_global_priority-features[]
  * [[{anchor-prefix}features-globalPriorityQuery]]
    pname:globalPriorityQuery indicates whether the implementation supports
    the ability to query global queue priorities.
// end::VK_KHR_global_priority-features[]

:refpage: VkPhysicalDeviceGlobalPriorityQueryFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceGlobalPriorityQueryFeatures.adoc[]
--
endif::VK_BASE_VERSION_1_4,VK_KHR_global_priority,VK_EXT_global_priority_query[]

ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_pipeline_creation_cache_control[]
[open,refpage='VkPhysicalDevicePipelineCreationCacheControlFeatures',desc='Structure describing whether pipeline cache control can be supported by an implementation',type='structs']
--
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_zero_initialize_workgroup_memory[]
[open,refpage='VkPhysicalDeviceZeroInitializeWorkgroupMemoryFeatures',desc='Structure describing support for zero initialization of workgroup memory by an implementation',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_3,VK_KHR_zero_initialize_workgroup_memory[]

ifdef::VK_BASE_VERSION_1_3,VK_EXT_private_data[]
[open,refpage='VkPhysicalDevicePrivateDataFeatures',desc='Structure specifying physical device support',type='structs']
--
...
[open,refpage='VkPhysicalDeviceRobustness2FeaturesKHR',desc='Structure describing the out-of-bounds behavior for an implementation',type='structs']
--
The sname:VkPhysicalDeviceRobustness2FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceRobustness2FeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-robustBufferAccess2]] pname:robustBufferAccess2 enables
    <<shaders-robust-buffer-access2>> guarantees for shader buffer accesses.
  * [[features-robustImageAccess2]] pname:robustImageAccess2 enables
    <<shaders-robust-image-access2>> guarantees for shader image accesses.
...
:refpage: VkPhysicalDeviceRobustness2FeaturesKHR
include::{chapters}/features.adoc[tag=features]
...
include::{generated}/validity/structs/VkPhysicalDeviceRobustness2FeaturesKHR.adoc[]
--
endif::VK_EXT_robustness2,VK_KHR_robustness2[]

ifndef::VK_EXT_robustness2+VK_KHR_robustness2[]
[[features-nullDescriptor]] nullDescriptor support requires the
`apiext:VK_KHR_robustness2` or `apiext:VK_EXT_robustness2` extension.
endif::VK_EXT_robustness2+VK_KHR_robustness2[]

ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_image_robustness[]
[open,refpage='VkPhysicalDeviceImageRobustnessFeatures',desc='Structure describing the out-of-bounds behavior for an implementation',type='structs']
--
...
// tag::VK_EXT_image_robustness-features[]
  * [[{anchor-prefix}features-robustImageAccess]] pname:robustImageAccess
    enables <<shaders-robust-image-access>> guarantees for shader image
    accesses.
...
--
endif::VK_COMPUTE_VERSION_1_3,VK_EXT_image_robustness[]

ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_shader_terminate_invocation[]
[open,refpage='VkPhysicalDeviceShaderTerminateInvocationFeatures',desc='Structure describing support for the SPIR-V code:SPV_KHR_terminate_invocation extension',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_3,VK_KHR_shader_terminate_invocation[]
...
This structure describes the following features:
  ...
  * [[features-mutableComparisonSamplers]] pname:mutableComparisonSamplers
    ...
    samplers to be <<descriptors-sets-updates, updated>>.
  ...
  * [[features-shaderSampleRateInterpolationFunctions]]
    ...
    code:InterpolateAtCentroid, code:InterpolateAtOffset, and
    code:InterpolateAtSample from the `GLSL.std.450` extended instruction
    set.
  ...
  * [[features-tessellationIsolines]] pname:tessellationIsolines indicates
    ...
    This member is only meaningful if the <<features-tessellationShader,
    pname:tessellationShader>> feature is supported.
...
[open,refpage='VkPhysicalDeviceMutableDescriptorTypeFeaturesEXT',desc='Structure describing whether the mutable descriptor type is supported',type='structs']
--
...
ifdef::VK_EXT_untyped_pointers[]
    objects unless the <<features-shaderUntypedPointersWorkgroup,
    pname:shaderUntypedPointersWorkgroup>> feature is enabled and they are
    accessed in 32-bit multiples or 16-bit multiples if the
    <<features-workgroupMemoryExplicitLayout16BitAccess,
    pname:workgroupMemoryExplicitLayout16BitAccess>> feature is enabled.
endif::VK_EXT_untyped_pointers[]
    This also indicates whether shader modules can: declare the
...
ifdef::VK_EXT_untyped_pointers[]
    objects unless the <<features-workgroupMemoryExplicitLayout8BitAccess,
    pname:workgroupMemoryExplicitLayout8BitAccess>> feature is enabled or
    they are accessed in 32-bit multiples and the
    <<features-shaderUntypedPointersWorkgroup,
    pname:shaderUntypedPointersWorkgroup>> feature is enabled.
endif::VK_EXT_untyped_pointers[]
    This also indicates whether shader modules can: declare the
...
[open,refpage='VkPhysicalDeviceSynchronization2Features',desc='Structure describing whether the implementation supports v2 synchronization commands',type='structs']
--
...
ifdef::VK_KHR_shader_untyped_pointers[]
[open,refpage='VkPhysicalDeviceShaderUntypedPointersFeaturesKHR', desc='Structure describing support for untyped pointers in shader by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderUntypedPointersFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderUntypedPointersFeaturesKHR.adoc[]

The members of sname:VkPhysicalDeviceShaderUntypedPointersFeaturesKHR
describe the following features:

  * [[features-shaderUntypedPointers]] pname:shaderUntypedPointers specifies
    whether shader modules can: declare the code:UntypedPointersKHR
    capability and untyped pointers in any
    <<interfaces-resources-layout,explicitly laid out storage class>>.

If the sname:VkPhysicalDeviceShaderUntypedPointersFeaturesKHR structure is
...
sname:VkPhysicalDeviceShaderUntypedPointersFeaturesKHR can: also be included
in the pname:pNext chain of slink:VkDeviceCreateInfo to enable the features.

include::{generated}/validity/structs/VkPhysicalDeviceShaderUntypedPointersFeaturesKHR.adoc[]
--
endif::VK_KHR_shader_untyped_pointers[]
...
[open,refpage='VkPhysicalDevicePipelineProtectedAccessFeatures',desc='Structure describing support for specifying protected access on individual pipelines',type='structs']
--
The sname:VkPhysicalDevicePipelineProtectedAccessFeatures structure is
defined as:

include::{generated}/api/structs/VkPhysicalDevicePipelineProtectedAccessFeatures.adoc[]
...
// Must have preceding whitespace
ifdef::VK_COMPUTE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_4[:anchor-prefix:]
// tag::VK_EXT_pipeline_protected_access-features[]
  * [[{anchor-prefix}features-pipelineProtectedAccess]]
    pname:pipelineProtectedAccess indicates whether the implementation
    supports specifying protected access on individual pipelines.
// end::VK_EXT_pipeline_protected_access-features[]

:refpage: VkPhysicalDevicePipelineProtectedAccessFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePipelineProtectedAccessFeatures.adoc[]
--
...
This structure describes the following feature:
  ...
  * [[features-ycbcr2plane444Formats]] pname:ycbcr2plane444Formats indicates
    ...
    using, the following 2-plane 444 {YCbCr} formats:
...
ifdef::VK_ARM_tensors[]
[open,refpage='VkPhysicalDeviceDescriptorBufferTensorFeaturesARM',desc='Structure describing the descriptor buffer tensor features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceDescriptorBufferTensorFeaturesARM structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceDescriptorBufferTensorFeaturesARM.adoc[]
...
This structure describes the following features:
  ...
  * [[features-descriptorBufferTensorDescriptors]]
    pname:descriptorBufferTensorDescriptors indicates that the
    implementation supports putthing shader-accessible tensor descriptors
    directly in memory.

:refpage: VkPhysicalDeviceDescriptorBufferTensorFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDescriptorBufferTensorFeaturesARM.adoc[]
--
endif::VK_ARM_tensors[]
...
[open,refpage='VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV',desc='Structure describing the ray tracing linear swept spheres features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-spheres]] pname:spheres indicates whether the implementation
    supports sphere primitives in ray tracing.
  * [[features-linearSweptSpheres]] pname:linearSweptSpheres indicates
    whether the implementation supports linear swept sphere primitives in
    ray tracing.

:refpage: VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceRayTracingLinearSweptSpheresFeaturesNV.adoc[]
--
endif::VK_NV_ray_tracing_linear_swept_spheres[]

ifdef::VK_KHR_opacity_micromap[]
[open,refpage='VkPhysicalDeviceOpacityMicromapFeaturesKHR',desc='Structure describing the ray tracing opacity micromap features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceOpacityMicromapFeaturesKHR structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceOpacityMicromapFeaturesKHR.adoc[]
...
:refpage: VkPhysicalDeviceOpacityMicromapFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceOpacityMicromapFeaturesKHR.adoc[]
--
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-micromapEXT]] pname:micromap indicates whether the
    implementation supports the micromap array feature.
  * [[features-micromapCaptureReplayEXT]] pname:micromapCaptureReplay
    indicates whether the implementation supports capture and replay of
    addresses for micromap arrays.
  * [[features-micromapHostCommandsEXT]] pname:micromapHostCommands
...
[open,refpage='VkPhysicalDeviceZeroInitializeDeviceMemoryFeaturesEXT',desc='Structure describing whether the implementation supports cleared allocation functionality',type='structs']
--
The sname:VkPhysicalDeviceZeroInitializeDeviceMemoryFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceZeroInitializeDeviceMemoryFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-zeroInitializeDeviceMemory]] pname:zeroInitializeDeviceMemory
    indicates that the implementation supports zeroing memory allocations
    using a user-specified flag.

:refpage: VkPhysicalDeviceZeroInitializeDeviceMemoryFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceZeroInitializeDeviceMemoryFeaturesEXT.adoc[]
--
...
[open,refpage='VkPhysicalDevicePresentId2FeaturesKHR',desc='Structure indicating support for present id 2',type='structs']
--
The sname:VkPhysicalDevicePresentId2FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDevicePresentId2FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-presentId2]] pname:presentId2 indicates that the
    implementation supports specifying present ID values in the
    sname:VkPresentId2KHR extension to the sname:VkPresentInfoKHR struct.

:refpage: VkPhysicalDevicePresentId2FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePresentId2FeaturesKHR.adoc[]
--
...
[open,refpage='VkPhysicalDevicePresentWait2FeaturesKHR',desc='Structure indicating support for present wait 2',type='structs']
--
The sname:VkPhysicalDevicePresentWait2FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDevicePresentWait2FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-presentWait2]] pname:presentWait2 indicates that the
    implementation supports fname:vkWaitForPresent2KHR.

:refpage: VkPhysicalDevicePresentWait2FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePresentWait2FeaturesKHR.adoc[]
--
endif::VK_KHR_present_wait2[]

ifdef::VK_KHR_unified_image_layouts[]
[open,refpage='VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR',desc='Structure describing whether the implementation provides unified image layouts',type='structs']
--
The sname:VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-unifiedImageLayouts]] pname:unifiedImageLayouts specifies
    whether usage of ename:VK_IMAGE_LAYOUT_GENERAL, where valid, incurs no
    loss in efficiency.
...
:refpage: VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceUnifiedImageLayoutsFeaturesKHR.adoc[]
--
endif::VK_KHR_unified_image_layouts[]

ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
[open,refpage='VkPhysicalDeviceHostImageCopyFeatures',desc='Structure indicating support for copies to or from images from host memory',type='structs']
--
The sname:VkPhysicalDeviceHostImageCopyFeatures structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceHostImageCopyFeatures.adoc[]
...
ifdef::VK_BASE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_4[:anchor-prefix:]
// tag::VK_EXT_host_image_copy-features[]
  * [[{anchor-prefix}features-hostImageCopy]] pname:hostImageCopy indicates
    that the implementation supports copying from host memory to images
    using the flink:vkCopyMemoryToImage command, copying from images to host
    memory using the flink:vkCopyImageToMemory command, and copying between
    images using the flink:vkCopyImageToImage command.
// end::VK_EXT_host_image_copy-features[]

:refpage: VkPhysicalDeviceHostImageCopyFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceHostImageCopyFeatures.adoc[]
--
...
[open,refpage='VkPhysicalDeviceShaderFmaFeaturesKHR',desc='Structure indicating support for SPV_KHR_fma OpFmaKHR',type='structs']
--
The sname:VkPhysicalDeviceShaderFmaFeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderFmaFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderFmaFloat16]] pname:shaderFmaFloat16 indicates whether
    the implementation supports code:OpFmaKHR for Float16 types in shaders.
  * [[features-shaderFmaFloat32]] pname:shaderFmaFloat32 indicates whether
    the implementation supports code:OpFmaKHR for Float32 types in shaders.
  * [[features-shaderFmaFloat64]] pname:shaderFmaFloat64 indicates whether
    the implementation supports code:OpFmaKHR for Float64 types in shaders.

:refpage: VkPhysicalDeviceShaderFmaFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderFmaFeaturesKHR.adoc[]
--
endif::VK_KHR_shader_fma[]


ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_shader_integer_dot_product[]
[open,refpage='VkPhysicalDeviceShaderIntegerDotProductFeatures',desc='Structure describing integer dot product features that can be supported by an implementation',type='structs']
--
...
--
endif::VK_COMPUTE_VERSION_1_3,VK_KHR_shader_integer_dot_product[]

ifdef::VK_BASE_VERSION_1_3,VK_KHR_maintenance4[]
[open,refpage='VkPhysicalDeviceMaintenance4Features',desc='Structure describing whether the implementation supports maintenance4 functionality',type='structs']
--
...
[open,refpage='VkPhysicalDeviceMaintenance5Features',desc='Structure describing whether the implementation supports maintenance5 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance5Features structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance5Features.adoc[]
...
ifdef::VK_BASE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_maintenance5-features[]
  * [[{anchor-prefix}features-maintenance5]] pname:maintenance5 indicates
    that the implementation supports the following:
  ** The ability to expose support for the optional format
     ename:VK_FORMAT_A1B5G5R5_UNORM_PACK16.
  ** The ability to expose support for the optional format
     ename:VK_FORMAT_A8_UNORM.
  ...
  ** Creating a sname:VkBufferView with a subset of the associated
     sname:VkBuffer usage using slink:VkBufferUsageFlags2CreateInfo.
  ** A new function flink:vkCmdBindIndexBuffer2, allowing a range of memory
  ...
  ** A function flink:vkGetRenderingAreaGranularity to query the optimal
  ...
  ** flink:vkGetDeviceImageSubresourceLayout allows an application to
  ...
  ** Two properties to indicate the non-strict line rasterization algorithm
     used.
  ** Two new flags words elink:VkPipelineCreateFlagBits2 and
     elink:VkBufferUsageFlagBits2.
...
// end::VK_KHR_maintenance5-features[]

:refpage: VkPhysicalDeviceMaintenance5Features
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance5Features.adoc[]
--
endif::VK_BASE_VERSION_1_4,VK_KHR_maintenance5[]

ifdef::VK_BASE_VERSION_1_4,VK_KHR_maintenance6[]
[open,refpage='VkPhysicalDeviceMaintenance6Features',desc='Structure describing whether the implementation supports maintenance6 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance6Features structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance6Features.adoc[]
...
ifdef::VK_BASE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_BASE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_maintenance6-features[]
  * [[{anchor-prefix}features-maintenance6]] pname:maintenance6 indicates
    that the implementation supports the following:
  ** dlink:VK_NULL_HANDLE can: be used when binding an index buffer
  ** slink:VkBindMemoryStatus can: be included in the pname:pNext chain of
     the slink:VkBindBufferMemoryInfo and slink:VkBindImageMemoryInfo
     structures, enabling applications to retrieve elink:VkResult values for
     individual memory binding operations.
  ** slink:VkPhysicalDeviceMaintenance6Properties::pname:blockTexelViewCompatibleMultipleLayers
  ...
  ** A
     slink:VkPhysicalDeviceMaintenance6Properties::pname:fragmentShadingRateClampCombinerInputs
     property which indicates whether the implementation clamps the inputs
     to fragment shading rate combiner operations.
// end::VK_KHR_maintenance6-features[]

:refpage: VkPhysicalDeviceMaintenance6Features
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance6Features.adoc[]
--
...
[open,refpage='VkPhysicalDeviceMaintenance8FeaturesKHR',desc='Structure describing whether the implementation supports maintenance8 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance8FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance8FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-maintenance8]] pname:maintenance8 indicates that the
    implementation supports the following:
  ** Allow copies between depth/stencil and "`matching`" color attachments
  ** Allow pname:dstCache in flink:vkMergePipelineCaches to be implicitly
     synchronized.
  ** Require src/dst sync scopes to work when doing queue family ownership
     transfers
  ** Support code:Offset (as an alternative to code:ConstOffset) image
  ...
  ** Add space for an additional 64 access flags for use with
     slink:VkMemoryBarrier2, slink:VkBufferMemoryBarrier2, and
     slink:VkImageMemoryBarrier2

:refpage: VkPhysicalDeviceMaintenance8FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance8FeaturesKHR.adoc[]
--
endif::VK_KHR_maintenance8[]

ifdef::VK_KHR_maintenance9[]
[open,refpage='VkPhysicalDeviceMaintenance9FeaturesKHR',desc='Structure describing whether the implementation supports maintenance9 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance9FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance9FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-maintenance9]] pname:maintenance9 indicates that the
    implementation supports the following:
  ...
  ** The value returned when a vertex shader reads an unbound vertex
     ...
     <<limits-defaultVertexAttributeValue,
     pname:defaultVertexAttributeValue>> property.
  ** A new
     ...
     state on creation.
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
  ** flink:vkCmdSetEvent2 may: not provide a dependency other than the event
     src stage mask.
endif::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
  ** The effects of image memory barriers and image layout transitions on 3D
     images created with ename:VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT are
     limited to only those slices specified in sname:VkImageSubresourceRange
  ...
  ** Queue family ownership transfers are no longer required for buffers and
     ...
     check if resources created with ename:VK_SHARING_MODE_EXCLUSIVE can:
     automatically transfer ownership between two queue families.
ifdef::VK_EXT_image_2d_view_of_3d[]
  ** <<limits-image2DViewOf3DSparse, pname:image2DViewOf3DSparse>> enables
     2D views of 3D sparse images.
endif::VK_EXT_image_2d_view_of_3d[]

:refpage: VkPhysicalDeviceMaintenance9FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance9FeaturesKHR.adoc[]
--
endif::VK_KHR_maintenance9[]

ifdef::VK_KHR_maintenance10[]
[open,refpage='VkPhysicalDeviceMaintenance10FeaturesKHR',desc='Structure describing whether the implementation supports maintenance10 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance10FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance10FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-maintenance10]] pname:maintenance10 indicates that the
    implementation supports the following:
...
ifdef::VK_EXT_extended_dynamic_state3,VK_EXT_shader_object[]
  ...
  ** Add flink:vkCmdEndRendering2KHR as an extensible version of
     flink:vkCmdEndRendering
  ...
  ** Add resolve mode and depth-stencil resolve support to
     flink:vkCmdResolveImage2 to bring it in-line with render pass
     attachment resolves

:refpage: VkPhysicalDeviceMaintenance10FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance10FeaturesKHR.adoc[]
--
endif::VK_KHR_maintenance10[]

ifdef::VK_KHR_maintenance11[]
[open,refpage='VkPhysicalDeviceMaintenance11FeaturesKHR',desc='Structure describing whether the implementation supports maintenance11 functionality',type='structs']
--
The sname:VkPhysicalDeviceMaintenance11FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMaintenance11FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-maintenance11]] pname:maintenance11 indicates that the
    implementation supports the following:
  ...
  ** The depth clipping state of the pipeline is well defined when the
     ...
     ename:VK_DYNAMIC_STATE_DEPTH_CLIP_ENABLE_EXT being set
ifdef::VK_EXT_depth_clip_enable[]
     and the slink:VkPipelineRasterizationDepthClipStateCreateInfoEXT
     structure is not present
endif::VK_EXT_depth_clip_enable[]
ifdef::VK_EXT_shader_object[]
  ** ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR
endif::VK_EXT_shader_object[]
  ** Allow pname:queueFamilyIndexCount of 1 in slink:VkBufferCreateInfo,
     slink:VkImageCreateInfo,
ifdef::VK_EXT_image_drm_format_modifier[slink:VkPhysicalDeviceImageDrmFormatModifierInfoEXT,]
ifdef::VK_ARM_tensors[slink:VkTensorCreateInfoARM]
     when pname:sharingMode is ename:VK_SHARING_MODE_CONCURRENT
  ** Require pname:minImageTransferGranularity to be [eq]#(1,1,1)# even on
     transfer-only queues and add pname:optimalImageTransferGranularity
...
:refpage: VkPhysicalDeviceMaintenance11FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMaintenance11FeaturesKHR.adoc[]
--
endif::VK_KHR_maintenance11[]

ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
[open,refpage='VkPhysicalDeviceDynamicRenderingFeatures',desc='Structure indicating support for dynamic render pass instances',type='structs']
--
...
// Must have preceding whitespace
ifdef::VK_GRAPHICS_VERSION_1_3[:anchor-prefix: extension-]
ifndef::VK_GRAPHICS_VERSION_1_3[:anchor-prefix:]
...
[open,refpage='VkPhysicalDeviceCustomResolveFeaturesEXT',desc='Structure indicating support for shader resolves',type='structs']
--
The sname:VkPhysicalDeviceCustomResolveFeaturesEXT structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceCustomResolveFeaturesEXT.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-customResolve]] pname:customResolve specifies that the
    implementation supports render pass resolves using shaders.

:refpage: VkPhysicalDeviceCustomResolveFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceCustomResolveFeaturesEXT.adoc[]
--
...
[open,refpage='VkPhysicalDevicePipelineRobustnessFeatures',desc='Structure describing whether an implementation supports robustness requests on a per-pipeline stage granularity',type='structs']
--
The sname:VkPhysicalDevicePipelineRobustnessFeatures structure is defined
as:

include::{generated}/api/structs/VkPhysicalDevicePipelineRobustnessFeatures.adoc[]
...
ifdef::VK_COMPUTE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_4[:anchor-prefix:]
// tag::VK_EXT_pipeline_robustness-features[]
  * [[{anchor-prefix}features-pipelineRobustness]] pname:pipelineRobustness
    indicates that robustness can: be requested on a per-pipeline-stage
    granularity.
// end::VK_EXT_pipeline_robustness-features[]

[NOTE]
====
Enabling the <<features-pipelineRobustness, pname:pipelineRobustness>>
...
features.
If robustness is not needed, the <<features-pipelineRobustness,
pname:pipelineRobustness>> feature should not be enabled by an application.
====

:refpage: VkPhysicalDevicePipelineRobustnessFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePipelineRobustnessFeatures.adoc[]
--
...
[open,refpage='VkPhysicalDeviceMultisampledRenderToSwapchainFeaturesEXT',desc='Structure describing whether multisampled rendering to single-sampled attachments is supported on swapchain images',type='structs']
--
The sname:VkPhysicalDeviceMultisampledRenderToSwapchainFeaturesEXT structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceMultisampledRenderToSwapchainFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-multisampledRenderToSwapchain]]
    pname:multisampledRenderToSwapchain indicates that the implementation
    supports multisampled rendering to single-sampled render pass
    attachments referencing swapchain images.

:refpage: VkPhysicalDeviceMultisampledRenderToSwapchainFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceMultisampledRenderToSwapchainFeaturesEXT.adoc[]
--
endif::VK_EXT_multisampled_render_to_swapchain[]
...
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_float_controls2[]
[open,refpage='VkPhysicalDeviceShaderFloatControls2Features',desc='Structure describing shader float controls 2 features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderFloatControls2Features structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderFloatControls2Features.adoc[]
...
ifdef::VK_COMPUTE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_shader_float_controls2-features[]
  * [[{anchor-prefix}features-shaderFloatControls2]]
    pname:shaderFloatControls2 specifies whether shader modules can: declare
    the code:FloatControls2 capability.
// end::VK_KHR_shader_float_controls2-features[]

:refpage: VkPhysicalDeviceShaderFloatControls2Features
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderFloatControls2Features.adoc[]
--
endif::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_float_controls2[]
...
ifdef::VK_QCOM_image_processing3[]
[open,refpage='VkPhysicalDeviceImageProcessing3FeaturesQCOM',desc='Structure describing image processing features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceImageProcessing3FeaturesQCOM structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceImageProcessing3FeaturesQCOM.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-imageGatherLinear]] pname:imageGatherLinear indicates that
    the implementation supports shader modules that declare the
    code:ImageGatherLinearQCOM capability.
  * [[features-imageGatherExtendedModes]] pname:imageGatherExtendedModes
    indicates that the implementation supports shader modules that declare
    the code:ImageGatherExtendedModesQCOM capability.
  * [[features-blockMatchExtendedClampToEdge]]
    pname:blockMatchExtendedClampToEdge indicates that the implementation
    extends support for ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE to both
    input images of all <<textures-blockmatch,block matching operations>>.

:refpage: VkPhysicalDeviceImageProcessing3FeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceImageProcessing3FeaturesQCOM.adoc[]
--
endif::VK_QCOM_image_processing3[]

ifdef::VK_EXT_depth_clamp_zero_one,VK_KHR_depth_clamp_zero_one[]
[open,refpage='VkPhysicalDeviceDepthClampZeroOneFeaturesKHR',desc='Structure describing feature to control zero to one depth clamping',type='structs']
--
The sname:VkPhysicalDeviceDepthClampZeroOneFeaturesKHR structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceDepthClampZeroOneFeaturesKHR.adoc[]
...
:refpage: VkPhysicalDeviceDepthClampZeroOneFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDepthClampZeroOneFeaturesKHR.adoc[]
--
endif::VK_EXT_depth_clamp_zero_one,VK_KHR_depth_clamp_zero_one[]
...
This structure describes the following features:
  ...
  * [[features-deviceFaultEXT]] pname:deviceFault indicates that the
    implementation supports the reporting of device fault information.
  * [[features-deviceFaultVendorBinaryEXT]] pname:deviceFaultVendorBinary
...
[open,refpage='VkPhysicalDeviceFaultFeaturesKHR',desc='Structure indicating support for device fault reporting',type='structs']
--
The sname:VkPhysicalDeviceFaultFeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceFaultFeaturesKHR.adoc[]
...
This structure describes the following features:
  ...
  * [[features-deviceFaultVendorBinary]] pname:deviceFaultVendorBinary
    ...
    These may: provide additional information when imported into
    vendor-specific external tools.
  * [[features-deviceFaultReportMasked]] pname:deviceFaultReportMasked
    ...
    receiving a ename:VK_ERROR_DEVICE_LOST error) which may: be reported via
    this extension [.bold]#even if they did not# result in a
    ename:VK_ERROR_DEVICE_LOST condition being returned to the application.
  * [[features-deviceFaultDeviceLostOnMasked]]
    pname:deviceFaultDeviceLostOnMasked indicates that the implementation
    supports returning ename:VK_ERROR_DEVICE_LOST for faults that would be
    normally be masked.
...
:refpage: VkPhysicalDeviceFaultFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceFaultFeaturesKHR.adoc[]
--
...
ifdef::VK_ARM_tensors[]
[open,refpage='VkPhysicalDeviceTensorFeaturesARM',desc='Structure describing tensor features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceTensorFeaturesARM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceTensorFeaturesARM.adoc[]

The members of the sname:VkPhysicalDeviceTensorFeaturesARM structure
describe the following features:

  * [[features-tensorNonPacked]] pname:tensorNonPacked indicates whether the
    implementation supports the creation of tensors that are not packed
    tensors.
  * [[features-shaderTensorAccess]] pname:shaderTensorAccess indicates
    whether shader modules can: declare the code:TensorsARM capability.
  * [[features-shaderStorageTensorArrayDynamicIndexing]]
    ...
    expressions in shader code.
    If this feature is not enabled, resources with a descriptor type of
    ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM must: be indexed only by constant
    integral expressions when aggregated into arrays in shader code.
    This also indicates whether shader modules can: declare the
    code:StorageTensorArrayDynamicIndexingARM capability.
  * [[features-shaderStorageTensorArrayNonUniformIndexing]]
    pname:shaderStorageTensorArrayNonUniformIndexing indicates whether
    arrays of storage tensors can: be indexed by non-uniform integer
    expressions in shader code.
    If this feature is not enabled, resources with a descriptor type of
    ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM must: not be indexed by non-uniform
    integer expressions when aggregated into arrays in shader code.
    This also indicates whether shader modules can: declare the
    code:StorageTensorArrayNonUniformIndexingARM capability.
  * [[features-descriptorBindingStorageTensorUpdateAfterBind]]
    pname:descriptorBindingStorageTensorUpdateAfterBind indicates whether
    the implementation supports updating storage tensor descriptors after a
    set is bound.
    If this feature is not enabled,
    ename:VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT must: not be used with
    ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM.
  * [[features-tensors]] pname:tensors indicates whether the implementation
    supports tensor resources.


:refpage: VkPhysicalDeviceTensorFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceTensorFeaturesARM.adoc[]
--
endif::VK_ARM_tensors[]
...
[open,refpage='VkPhysicalDeviceSwapchainMaintenance1FeaturesKHR',desc='Structure describing whether implementation supports swapchain maintenance1 functionality',type='structs']
--
The sname:VkPhysicalDeviceSwapchainMaintenance1FeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceSwapchainMaintenance1FeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * [[features-swapchainMaintenance1]] pname:swapchainMaintenance1 indicates
    that the implementation supports the following:
  ** slink:VkSwapchainPresentFenceInfoKHR, specifying a fence that is
     signaled when the resources associated with a present operation can: be
     safely destroyed.
  ** slink:VkSwapchainPresentModesCreateInfoKHR and
     slink:VkSwapchainPresentModeInfoKHR, allowing the swapchain to switch
     present modes without a need for recreation.
  ** slink:VkSwapchainPresentScalingCreateInfoKHR, specifying the scaling
     behavior of the swapchain in presence of window resizing.
  ** The ename:VK_SWAPCHAIN_CREATE_DEFERRED_MEMORY_ALLOCATION_BIT_KHR flag,
     allowing the implementation to defer the allocation of swapchain image
     memory until first acquisition.
  ** flink:vkReleaseSwapchainImagesKHR, allowing acquired swapchain images
     to be released without presenting them.

:refpage: VkPhysicalDeviceSwapchainMaintenance1FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceSwapchainMaintenance1FeaturesKHR.adoc[]
--
...
[open,refpage='VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR',desc='Structure describing support for VK_PRESENT_MODE_FIFO_LATEST_READY_KHR',type='structs']
--
The sname:VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR.adoc[]
...
include::{generated}/api/structs/VkPhysicalDevicePresentModeFifoLatestReadyFeaturesEXT.adoc[]
  ...
  * [[features-presentModeFifoLatestReady]] pname:presentModeFifoLatestReady
    ...
    ename:VK_PRESENT_MODE_FIFO_LATEST_READY_KHR present mode.

:refpage: VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePresentModeFifoLatestReadyFeaturesKHR.adoc[]
--
...
[open,refpage='VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT',desc='Structure describing feature to control ray tracing invocation reordering',type='structs']
--
The sname:VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT structure
is defined as:

include::{generated}/api/structs/VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT.adoc[]
...
:refpage: VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT.adoc[]
--
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-rayTracingInvocationReorderNV]]
    pname:rayTracingInvocationReorder indicates that the implementation
...
[open,refpage='VkPhysicalDeviceHdrVividFeaturesHUAWEI',desc='Structure describing whether HDR Vivid metadata is supported',type='structs']
--
The sname:VkPhysicalDeviceHdrVividFeaturesHUAWEI structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceHdrVividFeaturesHUAWEI.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-hdrVivid]] pname:hdrVivid specifies whether HDR Vivid
    metadata is supported.

:refpage: VkPhysicalDeviceHdrVividFeaturesHUAWEI
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceHdrVividFeaturesHUAWEI.adoc[]
--
...
This structure describes the following features:
  ...
  * [[features-cubicRangeClamp]] pname:cubicRangeClamp indicates that the
...
This structure describes the following feature:
  ...
  * [[features-selectableCubicWeights]] pname:selectableCubicWeights
...
This structure describes the following feature:
  ...
  * [[features-dynamicPipelineLayout]] pname:dynamicPipelineLayout indicates
    ...
    slink:VkBindDescriptorSetsInfo, slink:VkPushConstantsInfo,
    slink:VkPushDescriptorSetInfo,
    slink:VkPushDescriptorSetWithTemplateInfo,
...
[open,refpage='VkPhysicalDeviceTileShadingFeaturesQCOM',desc='Structure describing tile shading features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceTileShadingFeaturesQCOM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceTileShadingFeaturesQCOM.adoc[]

This structure describes the following features:

  * [[features-tileShading]] pname:tileShading indicates that the
    implementation supports <<renderpass-tile-shading,tile shading render
    pass>> instances.
  * [[features-tileShadingFragmentStage]] pname:tileShadingFragmentStage
    indicates that the implementation supports tile shading in the fragment
    stage.
  * [[features-tileShadingColorAttachments]]
    pname:tileShadingColorAttachments indicates that the implementation
    supports access to color attachments in a tile shader.
  * [[features-tileShadingDepthAttachments]]
    pname:tileShadingDepthAttachments indicates that the implementation
    supports access to depth aspect of depth stencil attachments.
  * [[features-tileShadingStencilAttachments]]
    pname:tileShadingStencilAttachments indicates that the implementation
    supports access to stencil aspect of depth stencil attachments.
  * [[features-tileShadingInputAttachments]]
    pname:tileShadingInputAttachments indicates that the implementation
    supports access to input attachments.
  * [[features-tileShadingSampledAttachments]]
    pname:tileShadingSampledAttachments indicates that the implementation
    supports access to sampling of tile attachments.
  * [[features-tileShadingPerTileDraw]] pname:tileShadingPerTileDraw
    indicates that the implementation supports the recording of vkCmdDraw*
    commands when <<renderpass-per-tile-execution-model, per-tile execution
    model>> is enabled.
  * [[features-tileShadingPerTileDispatch]] pname:tileShadingPerTileDispatch
    indicates that the implementation supports the recording of
    fname:vkCmdDispatch* commands within those regions of a command buffer
    where the <<renderpass-per-tile-execution-model, per-tile execution
    model>> is enabled.
  * [[features-tileShadingDispatchTile]] pname:tileShadingDispatchTile
    indicates that the implementation supports the recording of
    flink:vkCmdDispatchTileQCOM commands.
  * [[features-tileShadingApron]] pname:tileShadingApron indicates that the
    implementation supports
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:apronSize value other
    than (0,0).
    See <<renderpass-tile-shading-aprons,Tiling Aprons>> for more
    information.
  * [[features-tileShadingAnisotropicApron]]
    pname:tileShadingAnisotropicApron indicates that the implementation
    supports slink:VkRenderPassTileShadingCreateInfoQCOM::pname:apronSize
    set to a value where pname:apronSize.width differs from
    pname:apronSize.height.
  * [[features-tileShadingAtomicOps]] pname:tileShadingAtomicOps indicates
    that the implementation supports atomic operations on
    <<interfaces-tile-attachment, tile attachment variables>>.
  * [[features-tileShadingImageProcessing]] pname:tileShadingImageProcessing
    indicates that the implementation supports <<textures-weightimage,image
    processing operations>> with tile attachments.

:refpage: VkPhysicalDeviceTileShadingFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceTileShadingFeaturesQCOM.adoc[]
--
...
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_subgroup_rotate[]
[open,refpage='VkPhysicalDeviceShaderSubgroupRotateFeatures',desc='Structure describing whether subgroup rotation is enabled',type='structs']
--
The sname:VkPhysicalDeviceShaderSubgroupRotateFeatures structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderSubgroupRotateFeatures.adoc[]
...
ifdef::VK_COMPUTE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_shader_subgroup_rotate-features[]
  * [[{anchor-prefix}features-shaderSubgroupRotate]]
    pname:shaderSubgroupRotate specifies whether shader modules can: declare
    the code:GroupNonUniformRotateKHR capability.
  * [[{anchor-prefix}features-shaderSubgroupRotateClustered]]
...
// end::VK_KHR_shader_subgroup_rotate-features[]

:refpage: VkPhysicalDeviceShaderSubgroupRotateFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderSubgroupRotateFeatures.adoc[]
--
endif::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_subgroup_rotate[]

ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_expect_assume[]
[open,refpage='VkPhysicalDeviceShaderExpectAssumeFeatures',desc='Structure describing shader expect assume features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderExpectAssumeFeatures structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderExpectAssumeFeatures.adoc[]
...
ifdef::VK_COMPUTE_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_COMPUTE_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_shader_expect_assume-features[]
  * [[{anchor-prefix}features-shaderExpectAssume]] pname:shaderExpectAssume
    specifies whether shader modules can: declare the code:ExpectAssumeKHR
    capability.
// end::VK_KHR_shader_expect_assume-features[]

:refpage: VkPhysicalDeviceShaderExpectAssumeFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderExpectAssumeFeatures.adoc[]
--
endif::VK_COMPUTE_VERSION_1_4,VK_KHR_shader_expect_assume[]


ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_dynamic_rendering_local_read[]
[open,refpage='VkPhysicalDeviceDynamicRenderingLocalReadFeatures',desc='Structure indicating support for local reads in dynamic render pass instances',type='structs']
--
The sname:VkPhysicalDeviceDynamicRenderingLocalReadFeatures structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceDynamicRenderingLocalReadFeatures.adoc[]
...
ifdef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix: extension-]
ifndef::VK_GRAPHICS_VERSION_1_4[:anchor-prefix:]
// tag::VK_KHR_dynamic_rendering_local_read-features[]
  * [[{anchor-prefix}features-dynamicRenderingLocalRead]]
    pname:dynamicRenderingLocalRead specifies that the implementation
    supports local reads inside dynamic render pass instances using the
    flink:vkCmdBeginRendering command.
// end::VK_KHR_dynamic_rendering_local_read-features[]

:refpage: VkPhysicalDeviceDynamicRenderingLocalReadFeatures
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDynamicRenderingLocalReadFeatures.adoc[]
--
...
[open,refpage='VkPhysicalDeviceShaderBfloat16FeaturesKHR',desc='Structure describing bfloat16 features that can be supported by the implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderBfloat16FeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderBfloat16FeaturesKHR.adoc[]

This structure describes the following features:

  * [[features-shaderBFloat16Type]] pname:shaderBFloat16Type indicates
    whether the implementation supports shaders with the
    code:BFloat16TypeKHR capability.
  * [[features-shaderBFloat16DotProduct]] pname:shaderBFloat16DotProduct
    indicates whether the implementation supports shaders with the
    code:BFloat16DotProductKHR capability.
  * [[features-shaderBFloat16CooperativeMatrix]]
    pname:shaderBFloat16CooperativeMatrix indicates whether the
    implementation supports shaders with the
    code:BFloat16CooperativeMatrixKHR capability.

:refpage: VkPhysicalDeviceShaderBfloat16FeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderBfloat16FeaturesKHR.adoc[]
--
...
[open,refpage='VkPhysicalDevicePipelineOpacityMicromapFeaturesARM',desc='Structure describing features supported by VK_ARM_pipeline_opacity_micromap',type='structs']
--
The slink:VkPhysicalDevicePipelineOpacityMicromapFeaturesARM structure is
defined as:

include::{generated}/api/structs/VkPhysicalDevicePipelineOpacityMicromapFeaturesARM.adoc[]
...
// tag::VK_ARM_pipeline_opacity_micromap-features[]
  * [[features-pipelineOpacityMicromap]] pname:pipelineOpacityMicromap
...
// end::VK_ARM_pipeline_opacity_micromap-features[]

:refpage: VkPhysicalDevicePipelineOpacityMicromapFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePipelineOpacityMicromapFeaturesARM.adoc[]
--
...
[open,refpage='VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX',desc='Structure describing dense geometry format features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-denseGeometryFormat]] pname:denseGeometryFormat specifies
    whether the implementation supports DGF1 compressed geometry data.

:refpage: VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDenseGeometryFormatFeaturesAMDX.adoc[]
--
endif::VK_AMDX_dense_geometry_format[]

ifdef::VK_EXT_vertex_attribute_robustness[]
[open,refpage='VkPhysicalDeviceVertexAttributeRobustnessFeaturesEXT',desc='Structure describing whether the vertex attribute robustness feature is supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceVertexAttributeRobustnessFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceVertexAttributeRobustnessFeaturesEXT.adoc[]
...
:refpage: VkPhysicalDeviceVertexAttributeRobustnessFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceVertexAttributeRobustnessFeaturesEXT.adoc[]
--
endif::VK_EXT_vertex_attribute_robustness[]

ifdef::VK_QCOM_tile_memory_heap[]
[open,refpage='VkPhysicalDeviceTileMemoryHeapFeaturesQCOM',desc='Structure describing whether the tile memory heap features can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceTileMemoryHeapFeaturesQCOM structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceTileMemoryHeapFeaturesQCOM.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-tileMemoryHeap]] pname:tileMemoryHeap indicates whether the
    implementation supports tile memory heap functionality.

:refpage: VkPhysicalDeviceTileMemoryHeapFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceTileMemoryHeapFeaturesQCOM.adoc[]
--
endif::VK_QCOM_tile_memory_heap[]

ifdef::VK_QCOM_shader_multiple_wait_queues[]
[open,refpage='VkPhysicalDeviceShaderMultipleWaitQueuesFeaturesQCOM',desc='Structure describing whether shader multiple wait queues can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderMultipleWaitQueuesFeaturesQCOM structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderMultipleWaitQueuesFeaturesQCOM.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderMultipleWaitQueues]] pname:shaderMultipleWaitQueues
    indicates that the implementation supports SPIR-V modules declaring the
    code:MultipleWaitQueuesQCOM capability.

:refpage: VkPhysicalDeviceShaderMultipleWaitQueuesFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderMultipleWaitQueuesFeaturesQCOM.adoc[]
--
endif::VK_QCOM_shader_multiple_wait_queues[]
...
[open,refpage='VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT',desc='Structure describing uniform buffer unsized array features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT
structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT.adoc[]
...
This structure describes the following feature:
  ...
  * [[features-shaderUniformBufferUnsizedArray]]
    pname:shaderUniformBufferUnsizedArray indicates that the implementation
    supports declaring the last member of a uniform buffer block as an
    unsized array.

:refpage: VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderUniformBufferUnsizedArrayFeaturesEXT.adoc[]
--
endif::VK_EXT_shader_uniform_buffer_unsized_array[]

ifdef::VK_ARM_format_pack[]
[open,refpage='VkPhysicalDeviceFormatPackFeaturesARM',desc='Structure describing whether the additional formats feature is supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceFormatPackFeaturesARM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceFormatPackFeaturesARM.adoc[]

This structure describes the following feature:

  * [[features-formatPack]] pname:formatPack indicates that the
    implementation must: support using a elink:VkFormat of
    ename:VK_FORMAT_R10X6_UINT_PACK16_ARM,
    ename:VK_FORMAT_R10X6G10X6_UINT_2PACK16_ARM,
    ename:VK_FORMAT_R10X6G10X6B10X6A10X6_UINT_4PACK16_ARM,
    ename:VK_FORMAT_R12X4_UINT_PACK16_ARM,
    ename:VK_FORMAT_R12X4G12X4_UINT_2PACK16_ARM,
    ename:VK_FORMAT_R12X4G12X4B12X4A12X4_UINT_4PACK16_ARM,
    ename:VK_FORMAT_R14X2_UINT_PACK16_ARM,
    ename:VK_FORMAT_R14X2G14X2_UINT_2PACK16_ARM, and
    ename:VK_FORMAT_R14X2G14X2B14X2A14X2_UINT_4PACK16_ARM, with at least the
    following elink:VkFormatFeatureFlagBits:
  ** ename:VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT
  ** ename:VK_FORMAT_FEATURE_TRANSFER_SRC_BIT
  ** ename:VK_FORMAT_FEATURE_TRANSFER_DST_BIT

:refpage: VkPhysicalDeviceFormatPackFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceFormatPackFeaturesARM.adoc[]
--
endif::VK_ARM_format_pack[]

ifdef::VK_EXT_primitive_restart_index[]
[open,refpage='VkPhysicalDevicePrimitiveRestartIndexFeaturesEXT',desc='Structure describing features to configure primitive restart index',type='structs']
--
The sname:VkPhysicalDevicePrimitiveRestartIndexFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDevicePrimitiveRestartIndexFeaturesEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-primitiveRestartIndex]] pname:primitiveRestartIndex specifies
    whether the index for primitive restart can be set

:refpage: VkPhysicalDevicePrimitiveRestartIndexFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePrimitiveRestartIndexFeaturesEXT.adoc[]
--
endif::VK_EXT_primitive_restart_index[]

ifdef::VK_ARM_data_graph[]
[open,refpage='VkPhysicalDeviceDataGraphFeaturesARM',desc='Structure describing features to control data graph pipelines',type='structs']
--
The sname:VkPhysicalDeviceDataGraphFeaturesARM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceDataGraphFeaturesARM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-dataGraph]] pname:dataGraph specifies whether data graph
  ...
  * [[features-dataGraphUpdateAfterBind]] pname:dataGraphUpdateAfterBind
    ...
    slink:VkPipelineLayout that uses one or more slink:VkDescriptorSetLayout
  ...
  * [[features-dataGraphSpecializationConstants]]
    pname:dataGraphSpecializationConstants specifies whether data graph
  ...
  * [[features-dataGraphShaderModule]] pname:dataGraphShaderModule specifies
    whether data graph pipelines can: be created from a shader module.

:refpage: VkPhysicalDeviceDataGraphFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDataGraphFeaturesARM.adoc[]
--
endif::VK_ARM_data_graph[]

ifdef::VK_QCOM_data_graph_model[]
[open,refpage='VkPhysicalDeviceDataGraphModelFeaturesQCOM',desc='Structure describing features to control data graph model',type='structs']
--
The sname:VkPhysicalDeviceDataGraphModelFeaturesQCOM structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceDataGraphModelFeaturesQCOM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-dataGraphModelQCOM]] pname:dataGraphModel specifies whether
...
:refpage: VkPhysicalDeviceDataGraphModelFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDataGraphModelFeaturesQCOM.adoc[]
--
endif::VK_QCOM_data_graph_model[]

ifdef::VK_EXT_shader_float8[]
[open,refpage='VkPhysicalDeviceShaderFloat8FeaturesEXT',desc='Structure describing float8 features that can be supported by the implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderFloat8FeaturesEXT structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderFloat8FeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderFloat8]] pname:shaderFloat8 indicates whether the
    implementation supports shaders with the code:Float8EXT capability.
  * [[features-shaderFloat8CooperativeMatrix]]
    pname:shaderFloat8CooperativeMatrix indicates whether the implementation
    supports shaders with the code:Float8CooperativeMatrixEXT capability.

:refpage: VkPhysicalDeviceShaderFloat8FeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderFloat8FeaturesEXT.adoc[]
--
endif::VK_EXT_shader_float8[]

ifdef::VK_QCOM_queue_perf_hint[]
[open,refpage='VkPhysicalDeviceQueuePerfHintFeaturesQCOM',desc='Structure describing queue perf hint features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceQueuePerfHintFeaturesQCOM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceQueuePerfHintFeaturesQCOM.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-queuePerfHint]] pname:queuePerfHint specifies whether the
    implementation supports setting <<devsandqueues-perfhint,queue perf
    hints>>.

:refpage: VkPhysicalDeviceQueuePerfHintFeaturesQCOM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceQueuePerfHintFeaturesQCOM.adoc[]
--
endif::VK_QCOM_queue_perf_hint[]

ifdef::VK_EXT_shader_64bit_indexing[]
[open,refpage='VkPhysicalDeviceShader64BitIndexingFeaturesEXT',desc='Structure describing 64-bit indexing features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShader64BitIndexingFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShader64BitIndexingFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shader64BitIndexing]] pname:shader64BitIndexing indicates
    that the implementation supports <<spirvenv-64bindexing,using 64-bit
    address calculations>> for indexing
ifdef::VK_KHR_cooperative_matrix[cooperative matrices,]
ifdef::VK_NV_cooperative_vector[cooperative vectors,]
    storage buffers, and physical storage buffers.

:refpage: VkPhysicalDeviceShader64BitIndexingFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShader64BitIndexingFeaturesEXT.adoc[]
--
endif::VK_EXT_shader_64bit_indexing[]

ifdef::VK_EXT_shader_subgroup_partitioned[]
[open,refpage='VkPhysicalDeviceShaderSubgroupPartitionedFeaturesEXT',desc='Structure describing shader subgroup partitioned features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderSubgroupPartitionedFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderSubgroupPartitionedFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderSubgroupPartitioned]] pname:shaderSubgroupPartitioned
    indicates that the implementation supports
    <<features-subgroup-partitioned,VK_SUBGROUP_FEATURE_PARTITIONED_BIT_EXT>>.

:refpage: VkPhysicalDeviceShaderSubgroupPartitionedFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderSubgroupPartitionedFeaturesEXT.adoc[]
--
endif::VK_EXT_shader_subgroup_partitioned[]

ifdef::VK_EXT_shader_split_barrier[]
[open,refpage='VkPhysicalDeviceShaderSplitBarrierFeaturesEXT',desc='Structure describing the shader split barrier features that can be supported by an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderSplitBarrierFeaturesEXT structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderSplitBarrierFeaturesEXT.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderSplitBarrier]] pname:shaderSplitBarrier indicates that
    the implementation supports the code:SplitBarrierEXT SPIR-V capability.

:refpage: VkPhysicalDeviceShaderSplitBarrierFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderSplitBarrierFeaturesEXT.adoc[]
--
endif::VK_EXT_shader_split_barrier[]

ifdef::VK_ARM_performance_counters_by_region[]
[open,refpage='VkPhysicalDevicePerformanceCountersByRegionFeaturesARM',desc='Structure describing support for VK_ARM_performance_counters_by_region by an implementation',type='structs']
--
The sname:VkPhysicalDevicePerformanceCountersByRegionFeaturesARM structure
is defined as:

include::{generated}/api/structs/VkPhysicalDevicePerformanceCountersByRegionFeaturesARM.adoc[]
...
This structure describes the following features:
  ...
  * [[features-performanceCountersByRegion]]
    pname:performanceCountersByRegion specifies whether the implementation
    supports capture of per region performance counters.

See <<appendix-performance-counters-by-region-security, security model>> for
additional guarantees made by implementations that expose this feature.

:refpage: VkPhysicalDevicePerformanceCountersByRegionFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePerformanceCountersByRegionFeaturesARM.adoc[]
--
endif::VK_ARM_performance_counters_by_region[]

ifdef::VK_NV_compute_occupancy_priority[]
[open,refpage='VkPhysicalDeviceComputeOccupancyPriorityFeaturesNV',desc='Structure describing whether compute occupancy priority is supported by the implementation',type='structs']
--
The sname:VkPhysicalDeviceComputeOccupancyPriorityFeaturesNV structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceComputeOccupancyPriorityFeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-computeOccupancyPriority]] pname:computeOccupancyPriority is
    true if compute occupancy priority is supported.

:refpage: VkPhysicalDeviceComputeOccupancyPriorityFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceComputeOccupancyPriorityFeaturesNV.adoc[]
--
endif::VK_NV_compute_occupancy_priority[]

ifdef::VK_NV_push_constant_bank[]
[open,refpage='VkPhysicalDevicePushConstantBankFeaturesNV',desc='Structure describing push constant bank features',type='structs']
--
The sname:VkPhysicalDevicePushConstantBankFeaturesNV structure is defined
as:

include::{generated}/api/structs/VkPhysicalDevicePushConstantBankFeaturesNV.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-pushConstantBank]] pname:pushConstantBank indicates whether
    push constant bank functionality is supported.

:refpage: VkPhysicalDevicePushConstantBankFeaturesNV
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDevicePushConstantBankFeaturesNV.adoc[]
--
endif::VK_NV_push_constant_bank[]

ifdef::VK_EXT_descriptor_heap[]
[open,refpage='VkPhysicalDeviceDescriptorHeapFeaturesEXT',desc='Structure describing support for descriptor heaps',type='structs']
--
The sname:VkPhysicalDeviceDescriptorHeapFeaturesEXT structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceDescriptorHeapFeaturesEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-descriptorHeap]] pname:descriptorHeap specifies whether
    <<descriptorheaps, descriptor heaps>> can: be used.
  * [[features-descriptorHeapCaptureReplay]]
    pname:descriptorHeapCaptureReplay specifies whether
    <<descriptorheaps-writing,heap descriptors>> can: be captured and
    replayed.

:refpage: VkPhysicalDeviceDescriptorHeapFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDescriptorHeapFeaturesEXT.adoc[]
--
...
[open,refpage='VkPhysicalDeviceShaderInstrumentationFeaturesARM',desc='Structure describing support for shader instrumentation',type='structs']
--
The sname:VkPhysicalDeviceShaderInstrumentationFeaturesARM structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderInstrumentationFeaturesARM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderInstrumentation]] pname:shaderInstrumentation specifies
    whether shader instrumentation is supported.

:refpage: VkPhysicalDeviceShaderInstrumentationFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderInstrumentationFeaturesARM.adoc[]
--
endif::VK_ARM_shader_instrumentation[]

ifdef::VK_AMD_gpa_interface[]
[open,refpage='VkPhysicalDeviceGpaFeaturesAMD',desc='Structure describing support for GPU performance API',type='structs']
--
The sname:VkPhysicalDeviceGpaFeaturesAMD structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceGpaFeaturesAMD.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-perfCounters]] pname:perfCounters specifies whether
    performance counters are supported.
  * [[features-streamingPerfCounters]] pname:streamingPerfCounters specifies
    whether streaming performance counters are supported.
  * [[features-sqThreadTracing]] pname:sqThreadTracing specifies whether
    thread tracing is supported.
  * [[features-clockModes]] pname:clockModes specifies whether setting clock
    modes is supported.

:refpage: VkPhysicalDeviceGpaFeaturesAMD
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceGpaFeaturesAMD.adoc[]
--
...
--
The sname:VkPhysicalDeviceDeviceAddressCommandsFeaturesKHR structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceDeviceAddressCommandsFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-deviceAddressCommands]] pname:deviceAddressCommands specifies
    whether the implementation supports commands taking a
    basetype:VkDeviceAddress that would otherwise take a slink:VkBuffer
    object.

:refpage: VkPhysicalDeviceDeviceAddressCommandsFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDeviceAddressCommandsFeaturesKHR.adoc[]
--
endif::VK_KHR_device_address_commands[]


ifdef::VK_KHR_shader_constant_data[]
[open,refpage='VkPhysicalDeviceShaderConstantDataFeaturesKHR',desc='Structure describing support for VK_KHR_shader_constant_data in an implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderConstantDataFeaturesKHR structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceShaderConstantDataFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderConstantData]] pname:shaderConstantData specifies
    whether the implementation supports SPIR-V modules that use the
    code:VK_KHR_shader_constant_data extension.

:refpage: VkPhysicalDeviceShaderConstantDataFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderConstantDataFeaturesKHR.adoc[]
--
endif::VK_KHR_shader_constant_data[]

ifdef::VK_KHR_shader_abort[]
[open,refpage='VkPhysicalDeviceShaderAbortFeaturesKHR', desc='Structure describing whether shader abort is supported or not',type='structs']
--
The sname:VkPhysicalDeviceShaderAbortFeaturesKHR structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderAbortFeaturesKHR.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderAbort]] pname:shaderAbort specifies whether shaders
    can: declare the code:AbortKHR capability.

:refpage: VkPhysicalDeviceShaderAbortFeaturesKHR
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderAbortFeaturesKHR.adoc[]
--
endif::VK_KHR_shader_abort[]


ifdef::VK_ARM_data_graph_optical_flow[]
[open,refpage='VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM',desc='Structure describing the data graph optical flow features supported by the implementation',type='structs']
--
The sname:VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM.adoc[]

This structure describes the following feature:
  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-dataGraphOpticalFlow]] pname:dataGraphOpticalFlow indicates
    whether the implementation supports optical flow graph pipelines.

:refpage: VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDataGraphOpticalFlowFeaturesARM.adoc[]
--
endif::VK_ARM_data_graph_optical_flow[]

ifdef::VK_ARM_data_graph_neural_accelerator_statistics[]
[open,refpage='VkPhysicalDeviceDataGraphNeuralAcceleratorStatisticsFeaturesARM',desc='Structure describing features to control data graph neural accelerator statistics',type='structs']
--
The sname:VkPhysicalDeviceDataGraphNeuralAcceleratorStatisticsFeaturesARM
structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceDataGraphNeuralAcceleratorStatisticsFeaturesARM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-dataGraphNeuralAcceleratorStatistics]]
    pname:dataGraphNeuralAcceleratorStatistics indicates that the
    implementation supports gathering neural accelerator statistics for data
    graphs.

:refpage: VkPhysicalDeviceDataGraphNeuralAcceleratorStatisticsFeaturesARM
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceDataGraphNeuralAcceleratorStatisticsFeaturesARM.adoc[]
--
endif::VK_ARM_data_graph_neural_accelerator_statistics[]

ifdef::VK_EXT_image_tiling_control[]
[open,refpage='VkPhysicalDeviceImageTilingControlFeaturesEXT', desc='Structure describing features supported by VK_EXT_image_tiling_control',type='structs']
--
The sname:VkPhysicalDeviceImageTilingControlFeaturesEXT structure is defined
as:

include::{generated}/api/structs/VkPhysicalDeviceImageTilingControlFeaturesEXT.adoc[]
...
This structure describes the following feature:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-imageTilingControl]] pname:imageTilingControl indicates that
    the implementation supports control of optimal tiling arrangements at
    image creation time.

:refpage: VkPhysicalDeviceImageTilingControlFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceImageTilingControlFeaturesEXT.adoc[]
--
endif::VK_EXT_image_tiling_control[]

ifdef::VK_EXT_shader_ocp_microscaling_types[]
[open,refpage='VkPhysicalDeviceShaderOCPMicroscalingTypesFeaturesEXT',desc='Structure describing OCP microscaling types features that can be supported by the implementation',type='structs']
--
The sname:VkPhysicalDeviceShaderOCPMicroscalingTypesFeaturesEXT structure is
defined as:

include::{generated}/api/structs/VkPhysicalDeviceShaderOCPMicroscalingTypesFeaturesEXT.adoc[]
...
This structure describes the following features:
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * [[features-shaderFloat4]] pname:shaderFloat4 indicates whether the
    implementation supports shaders with the code:Float4EXT or
    code:BitcastExtractEXT capabilities.
  * [[features-shaderFloat6]] pname:shaderFloat6 indicates whether the
    implementation supports shaders with the code:Float6EXT or
    code:BitcastExtractEXT capabilities.
  * [[features-shaderFloat8UnsignedE8M0]] pname:shaderFloat8UnsignedE8M0
    indicates whether the implementation supports shaders with the
    code:Float8UnsignedE8M0EXT capability.
  * [[features-shaderMXInt8]] pname:shaderMXInt8 indicates whether the
    implementation supports shaders with the code:MXInt8EXT capability.

:refpage: VkPhysicalDeviceShaderOCPMicroscalingTypesFeaturesEXT
include::{chapters}/features.adoc[tag=features]

include::{generated}/validity/structs/VkPhysicalDeviceShaderOCPMicroscalingTypesFeaturesEXT.adoc[]
--
...
ifdef::VK_EXT_provoking_vertex[]
  * `apiext:VK_EXT_provoking_vertex`, if the
    <<features-extendedDynamicState3ProvokingVertexMode,
    pname:extendedDynamicState3ProvokingVertexMode>> feature is supported.
endif::VK_EXT_provoking_vertex[]
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_line_rasterization,VK_EXT_line_rasterization[]
  * Vulkan 1.4 or the apiext:VK_KHR_line_rasterization extension or the
...
ifdef::VK_EXT_depth_clip_control[]
  * `apiext:VK_EXT_depth_clip_control`, if the
    <<features-extendedDynamicState3DepthClipNegativeOneToOne,
...
ifdef::VK_GRAPHICS_VERSION_1_4[]
  * If Vulkan 1.4 is supported, at least one queue family is reported by
    flink:vkGetPhysicalDeviceQueueFamilyProperties that includes the
    ename:VK_QUEUE_GRAPHICS_BIT flag, and no additional queue families are
    reported that support the ename:VK_QUEUE_TRANSFER_BIT flag, the
    <<features-hostImageCopy,pname:hostImageCopy>> feature must: be
    supported.
...
feature may not be supported despite `apiext:VK_EXT_rgba10x6_formats` being
advertised.
====
